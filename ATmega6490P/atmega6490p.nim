# Peripheral access API for ATMEGA6490P microcontrollers (generated using svd2nim)
...
const LCD* = LCD_Type(
  ...
  LCDDR4: LCD_LCDDR4_Type(loc: 0xf0'u),
  ...
  LCDDR9: LCD_LCDDR9_Type(loc: 0xf5'u),
...
proc read*(reg: LCD_LCDCCR_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDCCR_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR4_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR4_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: LCD_LCDDR4_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc read*(reg: LCD_LCDDR9_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR9_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: LCD_LCDDR9_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
