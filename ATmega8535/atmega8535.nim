# Peripheral access API for ATMEGA8535 microcontrollers (generated using svd2nim)
...
# #### Device Peripheral Interrupts ############################################
  ...
  irqINT1              =    2 # External Interrupt 1
  ...
  irqSPI_STC           =   10 # SPI Serial Transfer Complete
  irqUSART_RX          =   11 # USART, RX Complete
  irqUSART_UDRE        =   12 # USART Data Register Empty
  irqUSART_TX          =   13 # USART, TX Complete
  irqADC               =   14 # ADC Conversion Complete
  irqEE_RDY            =   15 # EEPROM Ready
  irqANA_COMP          =   16 # Analog Comparator
  irqTWI               =   17 # Two-wire Serial Interface
  irqINT2              =   18 # External Interrupt Request 2
  irqTIMER0_COMP       =   19 # TimerCounter0 Compare Match
  irqSPM_RDY           =   20 # Store Program Memory Read
...
const TC0* = TC0_Type(
  SFIOR: TC0_SFIOR_Type(loc: 0x50'u),
...
proc write*(reg: CPU_MCUCR_Type, ISC0: uint8 = 0, ISC1: uint8 = 0, SM: CPU_MCUCR_SM = smIDLE, SE: bool = false) =
  var x: uint8
  x.setMask((ISC0 shl 0).masked(0 .. 1))
  x.setMask((ISC1 shl 2).masked(2 .. 3))
  x.setMask((SM.uint8 shl 4).masked(4 .. 7))
  x.setMask((SE.uint8 shl 6).masked(6 .. 6))
...
proc write*(reg: CPU_SFIOR_Type, PUD: bool = false, ACME: bool = false) =
  var x: uint8
...
func ISC0*(r: CPU_MCUCR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 1)

proc `ISC0=`*(r: var CPU_MCUCR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 1)
  tmp.setMask((val shl 0).masked(0 .. 1))
  r = tmp.CPU_MCUCR_Fields

func ISC1*(r: CPU_MCUCR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(2 .. 3)

proc `ISC1=`*(r: var CPU_MCUCR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 3)
...
func SM*(r: CPU_MCUCR_Fields): UncheckedEnum[CPU_MCUCR_SM] {.inline.} =
  toUncheckedEnum[CPU_MCUCR_SM](r.uint8.bitsliced(4 .. 7).int)
...
func ACME*(r: CPU_SFIOR_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `ACME=`*(r: var CPU_SFIOR_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.CPU_SFIOR_Fields
...
proc write*(reg: FUSE_HIGH_Type, BOOTRST: bool = false, BOOTSZ: FUSE_HIGH_BOOTSZ = x1024W_0C00, EESAVE: bool = false, CKOPT: bool = false, SPIEN: bool = false, WDTON: bool = false, S8535C: bool = false) =
  var x: uint8
  ...
  x.setMask((S8535C.uint8 shl 7).masked(7 .. 7))
...
func S8535C*(r: FUSE_HIGH_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `S8535C=`*(r: var FUSE_HIGH_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.FUSE_HIGH_Fields
...
proc write*(reg: TC0_SFIOR_Type, PSR10: bool = false) =
  var x: uint8
...
func PSR10*(r: TC0_SFIOR_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `PSR10=`*(r: var TC0_SFIOR_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
