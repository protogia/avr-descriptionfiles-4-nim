# Peripheral access API for ATMEGA329PA microcontrollers (generated using svd2nim)
...
type LCD_LCDDR00_Type* = object
  loc: uint

type LCD_LCDDR01_Type* = object
  loc: uint

type LCD_LCDDR02_Type* = object
  loc: uint

type LCD_LCDDR03_Type* = object
  loc: uint

type LCD_LCDDR05_Type* = object
  loc: uint

type LCD_LCDDR06_Type* = object
  loc: uint

type LCD_LCDDR07_Type* = object
  loc: uint

type LCD_LCDDR08_Type* = object
  loc: uint
...
type LCD_Type* = object
  ...
  LCDDR00*: LCD_LCDDR00_Type
  LCDDR01*: LCD_LCDDR01_Type
  LCDDR02*: LCD_LCDDR02_Type
  LCDDR03*: LCD_LCDDR03_Type
  LCDDR05*: LCD_LCDDR05_Type
  LCDDR06*: LCD_LCDDR06_Type
  LCDDR07*: LCD_LCDDR07_Type
  LCDDR08*: LCD_LCDDR08_Type
...
const LCD* = LCD_Type(
  ...
  LCDDR00: LCD_LCDDR00_Type(loc: 0xec'u),
  LCDDR01: LCD_LCDDR01_Type(loc: 0xed'u),
  LCDDR02: LCD_LCDDR02_Type(loc: 0xee'u),
  LCDDR03: LCD_LCDDR03_Type(loc: 0xef'u),
  LCDDR05: LCD_LCDDR05_Type(loc: 0xf1'u),
  LCDDR06: LCD_LCDDR06_Type(loc: 0xf2'u),
  LCDDR07: LCD_LCDDR07_Type(loc: 0xf3'u),
  LCDDR08: LCD_LCDDR08_Type(loc: 0xf4'u),
...
type
  ...
  LCD_LCDDR00_Fields* = distinct uint8
  LCD_LCDDR01_Fields* = distinct uint8
  LCD_LCDDR02_Fields* = distinct uint8
  LCD_LCDDR03_Fields* = distinct uint8
  LCD_LCDDR05_Fields* = distinct uint8
  LCD_LCDDR06_Fields* = distinct uint8
  LCD_LCDDR07_Fields* = distinct uint8
  LCD_LCDDR08_Fields* = distinct uint8
...
proc read*(reg: LCD_LCDDR00_Type): LCD_LCDDR00_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR00_Fields](reg.loc))

proc write*(reg: LCD_LCDDR00_Type, val: LCD_LCDDR00_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR00_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR00_Type, SEG000: bool = false, SEG001: bool = false, SEG002: bool = false, SEG003: bool = false, SEG004: bool = false, SEG005: bool = false, SEG006: bool = false, SEG007: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR00_Fields

template modifyIt*(reg: LCD_LCDDR00_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR01_Type): LCD_LCDDR01_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR01_Fields](reg.loc))

proc write*(reg: LCD_LCDDR01_Type, val: LCD_LCDDR01_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR01_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR01_Type, SEG008: bool = false, SEG009: bool = false, SEG010: bool = false, SEG011: bool = false, SEG012: bool = false, SEG013: bool = false, SEG014: bool = false, SEG015: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR01_Fields

template modifyIt*(reg: LCD_LCDDR01_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR02_Type): LCD_LCDDR02_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR02_Fields](reg.loc))

proc write*(reg: LCD_LCDDR02_Type, val: LCD_LCDDR02_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR02_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR02_Type, SEG016: bool = false, SEG017: bool = false, SEG018: bool = false, SEG019: bool = false, SEG020: bool = false, SEG021: bool = false, SEG022: bool = false, SEG023: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR02_Fields

template modifyIt*(reg: LCD_LCDDR02_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR03_Type): LCD_LCDDR03_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR03_Fields](reg.loc))

proc write*(reg: LCD_LCDDR03_Type, val: LCD_LCDDR03_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR03_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR03_Type, SEG024: bool = false, SEG025: bool = false, SEG026: bool = false, SEG027: bool = false, SEG028: bool = false, SEG029: bool = false, SEG030: bool = false, SEG031: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR03_Fields

template modifyIt*(reg: LCD_LCDDR03_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR05_Type): LCD_LCDDR05_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR05_Fields](reg.loc))

proc write*(reg: LCD_LCDDR05_Type, val: LCD_LCDDR05_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR05_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR05_Type, SEG100: bool = false, SEG101: bool = false, SEG102: bool = false, SEG103: bool = false, SEG104: bool = false, SEG105: bool = false, SEG106: bool = false, SEG107: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR05_Fields

template modifyIt*(reg: LCD_LCDDR05_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR06_Type): LCD_LCDDR06_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR06_Fields](reg.loc))

proc write*(reg: LCD_LCDDR06_Type, val: LCD_LCDDR06_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR06_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR06_Type, SEG108: bool = false, SEG109: bool = false, SEG110: bool = false, SEG111: bool = false, SEG112: bool = false, SEG113: bool = false, SEG114: bool = false, SEG115: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR06_Fields

template modifyIt*(reg: LCD_LCDDR06_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR07_Type): LCD_LCDDR07_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR07_Fields](reg.loc))

proc write*(reg: LCD_LCDDR07_Type, val: LCD_LCDDR07_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR07_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR07_Type, SEG116: bool = false, SEG117: bool = false, SEG118: bool = false, SEG119: bool = false, SEG120: bool = false, SEG121: bool = false, SEG122: bool = false, SEG123: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR07_Fields

template modifyIt*(reg: LCD_LCDDR07_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR08_Type): LCD_LCDDR08_Fields {.inline.} =
  volatileLoad(cast[ptr LCD_LCDDR08_Fields](reg.loc))

proc write*(reg: LCD_LCDDR08_Type, val: LCD_LCDDR08_Fields) {.inline.} =
  volatileStore(cast[ptr LCD_LCDDR08_Fields](reg.loc), val)

proc write*(reg: LCD_LCDDR08_Type, SEG124: bool = false, SEG125: bool = false, SEG126: bool = false, SEG127: bool = false, SEG128: bool = false, SEG129: bool = false, SEG130: bool = false, SEG131: bool = false) =
  var x: uint8
  ...
  reg.write x.LCD_LCDDR08_Fields

template modifyIt*(reg: LCD_LCDDR08_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
func SEG000*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG000=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR00_Fields

func SEG001*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG001=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR00_Fields

func SEG002*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG002=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR00_Fields

func SEG003*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG003=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR00_Fields

func SEG004*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG004=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR00_Fields

func SEG005*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG005=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR00_Fields

func SEG006*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG006=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR00_Fields

func SEG007*(r: LCD_LCDDR00_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG007=`*(r: var LCD_LCDDR00_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR00_Fields

func SEG008*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG008=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR01_Fields

func SEG009*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG009=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR01_Fields

func SEG010*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG010=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR01_Fields

func SEG011*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG011=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR01_Fields

func SEG012*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG012=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR01_Fields

func SEG013*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG013=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR01_Fields

func SEG014*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG014=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR01_Fields

func SEG015*(r: LCD_LCDDR01_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG015=`*(r: var LCD_LCDDR01_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR01_Fields

func SEG016*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG016=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR02_Fields

func SEG017*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG017=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR02_Fields

func SEG018*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG018=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR02_Fields

func SEG019*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG019=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR02_Fields

func SEG020*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG020=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR02_Fields

func SEG021*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG021=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR02_Fields

func SEG022*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG022=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR02_Fields

func SEG023*(r: LCD_LCDDR02_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG023=`*(r: var LCD_LCDDR02_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR02_Fields

func SEG024*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG024=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR03_Fields

func SEG025*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG025=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR03_Fields

func SEG026*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG026=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR03_Fields

func SEG027*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG027=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR03_Fields

func SEG028*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG028=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR03_Fields

func SEG029*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG029=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR03_Fields

func SEG030*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG030=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR03_Fields

func SEG031*(r: LCD_LCDDR03_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG031=`*(r: var LCD_LCDDR03_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR03_Fields

func SEG100*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG100=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR05_Fields

func SEG101*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG101=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR05_Fields

func SEG102*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG102=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR05_Fields

func SEG103*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG103=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR05_Fields

func SEG104*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG104=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR05_Fields

func SEG105*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG105=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR05_Fields

func SEG106*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG106=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR05_Fields

func SEG107*(r: LCD_LCDDR05_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG107=`*(r: var LCD_LCDDR05_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR05_Fields

func SEG108*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG108=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR06_Fields

func SEG109*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG109=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR06_Fields

func SEG110*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG110=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR06_Fields

func SEG111*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG111=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR06_Fields

func SEG112*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG112=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR06_Fields

func SEG113*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG113=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR06_Fields

func SEG114*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG114=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR06_Fields

func SEG115*(r: LCD_LCDDR06_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG115=`*(r: var LCD_LCDDR06_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR06_Fields

func SEG116*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG116=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR07_Fields

func SEG117*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG117=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR07_Fields

func SEG118*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG118=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR07_Fields

func SEG119*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG119=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR07_Fields

func SEG120*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG120=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR07_Fields

func SEG121*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG121=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR07_Fields

func SEG122*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG122=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR07_Fields

func SEG123*(r: LCD_LCDDR07_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG123=`*(r: var LCD_LCDDR07_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR07_Fields

func SEG124*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SEG124=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.LCD_LCDDR08_Fields

func SEG125*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SEG125=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.LCD_LCDDR08_Fields

func SEG126*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `SEG126=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.LCD_LCDDR08_Fields

func SEG127*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SEG127=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.LCD_LCDDR08_Fields

func SEG128*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `SEG128=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.LCD_LCDDR08_Fields

func SEG129*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SEG129=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.LCD_LCDDR08_Fields

func SEG130*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SEG130=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.LCD_LCDDR08_Fields

func SEG131*(r: LCD_LCDDR08_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SEG131=`*(r: var LCD_LCDDR08_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.LCD_LCDDR08_Fields
