# Peripheral access API for ATMEGA48PA microcontrollers (generated using svd2nim)
...
const EEPROM* = EEPROM_Type(
  ...
  EEARH: EEPROM_EEARH_Type(loc: 0x42'u),
)
...
type
  EEPROM_EEARH_Fields* = distinct uint8
...
proc read*(reg: EEPROM_EEARH_Type): EEPROM_EEARH_Fields {.inline.} =
  volatileLoad(cast[ptr EEPROM_EEARH_Fields](reg.loc))

proc write*(reg: EEPROM_EEARH_Type, val: EEPROM_EEARH_Fields) {.inline.} =
  volatileStore(cast[ptr EEPROM_EEARH_Fields](reg.loc), val)

proc write*(reg: EEPROM_EEARH_Type, EEAR8: bool = false, EEAR9: bool = false) =
  var x: uint8
  x.setMask((EEAR8.uint8 shl 0).masked(0 .. 0))
  x.setMask((EEAR9.uint8 shl 1).masked(1 .. 1))
  reg.write x.EEPROM_EEARH_Fields

template modifyIt*(reg: EEPROM_EEARH_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
func EEAR8*(r: EEPROM_EEARH_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `EEAR8=`*(r: var EEPROM_EEARH_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.EEPROM_EEARH_Fields

func EEAR9*(r: EEPROM_EEARH_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `EEAR9=`*(r: var EEPROM_EEARH_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.EEPROM_EEARH_Fields
