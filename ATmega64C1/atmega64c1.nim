# Peripheral access API for ATMEGA64C1 microcontrollers (generated using svd2nim)
...
type CAN_CANSTMH_Type* = object
  loc: uint

type CAN_CANSTML_Type* = object
  loc: uint
...
type CAN_CANTIMH_Type* = object
  loc: uint

type CAN_CANTIML_Type* = object
  loc: uint

type CAN_CANTTCH_Type* = object
  loc: uint

type CAN_CANTTCL_Type* = object
  loc: uint
...
type CAN_Type* = object
  ...
  CANTIML*: CAN_CANTIML_Type
  CANTIMH*: CAN_CANTIMH_Type
  CANTTCL*: CAN_CANTTCL_Type
  CANTTCH*: CAN_CANTTCH_Type
  ...
  CANSTML*: CAN_CANSTML_Type
  CANSTMH*: CAN_CANSTMH_Type
...
const CAN* = CAN_Type(
  ...
  CANTIML: CAN_CANTIML_Type(loc: 0xe6'u),
  CANTIMH: CAN_CANTIMH_Type(loc: 0xe7'u),
  CANTTCL: CAN_CANTTCL_Type(loc: 0xe8'u),
  CANTTCH: CAN_CANTTCH_Type(loc: 0xe9'u),
  ...
  CANSTML: CAN_CANSTML_Type(loc: 0xf8'u),
  CANSTMH: CAN_CANSTMH_Type(loc: 0xf9'u),
...
const DAC* = DAC_Type(
  ...
  DACL: DAC_DACL_Type(loc: 0x91'u),
  DACH: DAC_DACH_Type(loc: 0x92'u),
)
...
const LINUART* = LINUART_Type(
  ...
  LINBRRL: LINUART_LINBRRL_Type(loc: 0xcd'u),
  LINBRRH: LINUART_LINBRRH_Type(loc: 0xce'u),
...
proc read*(reg: CAN_CANIDM4_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: CAN_CANIDM4_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: CAN_CANSTMH_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: CAN_CANSTMH_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: CAN_CANSTMH_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: CAN_CANSTML_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: CAN_CANSTML_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: CAN_CANSTML_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc read*(reg: CAN_CANTIMH_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: CAN_CANTIMH_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: CAN_CANTIMH_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: CAN_CANTIML_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: CAN_CANTIML_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: CAN_CANTIML_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: CAN_CANTTCH_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: CAN_CANTTCH_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: CAN_CANTTCH_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: CAN_CANTTCL_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: CAN_CANTTCL_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: CAN_CANTTCL_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
