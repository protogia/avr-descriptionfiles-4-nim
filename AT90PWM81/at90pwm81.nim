# Peripheral access API for AT90PWM81 microcontrollers (generated using svd2nim)
...
const PSC2* = PSC2_Type(
  ...
  PICR2: PSC2_PICR2_Type(loc: 0x6c'u),
...
proc write*(reg: ADC_DIDR0_Type, ADC0D: bool = false, ADC1D: bool = false, ADC2D: bool = false, ADC3D: bool = false, ADC4D: bool = false, ADC5D: bool = false, ADC7D: bool = false, ADC8D: bool = false) =
  var x: uint8
  ...
  x.setMask((ADC7D.uint8 shl 6).masked(6 .. 6))
  x.setMask((ADC8D.uint8 shl 7).masked(7 .. 7))
...
proc write*(reg: ADC_DIDR1_Type, ADC9D: bool = false, ADC10D: bool = false, AMP0PD: bool = false, ACMP1MD: bool = false) =
  var x: uint8
  x.setMask((ADC9D.uint8 shl 0).masked(0 .. 0))
  x.setMask((ADC10D.uint8 shl 1).masked(1 .. 1))
  x.setMask((AMP0PD.uint8 shl 2).masked(2 .. 2))
...
func ADC8D*(r: ADC_DIDR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `ADC8D=`*(r: var ADC_DIDR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.ADC_DIDR0_Fields
...
type
  DAC_DAC_Fields* = distinct uint16
...
proc read*(reg: DAC_DAC_Type): DAC_DAC_Fields {.inline.} =
  volatileLoad(cast[ptr DAC_DAC_Fields](reg.loc))

proc write*(reg: DAC_DAC_Type, val: DAC_DAC_Fields) {.inline.} =
  volatileStore(cast[ptr DAC_DAC_Fields](reg.loc), val)

proc write*(reg: DAC_DAC_Type, DACH: uint16 = 0) =
  var x: uint16
  x.setMask((DACH shl 0).masked(0 .. 9))
  reg.write x.DAC_DAC_Fields
...
func DACH*(r: DAC_DAC_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 9)

proc `DACH=`*(r: var DAC_DAC_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 9)
  tmp.setMask((val shl 0).masked(0 .. 9))
  r = tmp.DAC_DAC_Fields
