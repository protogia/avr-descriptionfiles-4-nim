# Peripheral access API for ATMEGA48PB microcontrollers (generated using svd2nim)
...
# #### Device Peripheral Interrupts ############################################
  ...
  irqUSART_START       =   26 # USART Start Edge Interrupt
...
type DEVICEID_DEVID0_Type* = object
  loc: uint

type DEVICEID_DEVID1_Type* = object
  loc: uint

type DEVICEID_DEVID2_Type* = object
  loc: uint

type DEVICEID_DEVID3_Type* = object
  loc: uint

type DEVICEID_DEVID4_Type* = object
  loc: uint

type DEVICEID_DEVID5_Type* = object
  loc: uint

type DEVICEID_DEVID6_Type* = object
  loc: uint

type DEVICEID_DEVID7_Type* = object
  loc: uint

type DEVICEID_DEVID8_Type* = object
  loc: uint

type DEVICEID_Type* = object
  DEVID0*: DEVICEID_DEVID0_Type
  DEVID1*: DEVICEID_DEVID1_Type
  DEVID2*: DEVICEID_DEVID2_Type
  DEVID3*: DEVICEID_DEVID3_Type
  DEVID4*: DEVICEID_DEVID4_Type
  DEVID5*: DEVICEID_DEVID5_Type
  DEVID6*: DEVICEID_DEVID6_Type
  DEVID7*: DEVICEID_DEVID7_Type
  DEVID8*: DEVICEID_DEVID8_Type

type EEPROM_EEARL_Type* = object
  loc: uint
...
const DEVICEID* = DEVICEID_Type(
  DEVID0: DEVICEID_DEVID0_Type(loc: 0xf0'u),
  DEVID1: DEVICEID_DEVID1_Type(loc: 0xf1'u),
  DEVID2: DEVICEID_DEVID2_Type(loc: 0xf2'u),
  DEVID3: DEVICEID_DEVID3_Type(loc: 0xf3'u),
  DEVID4: DEVICEID_DEVID4_Type(loc: 0xf4'u),
  DEVID5: DEVICEID_DEVID5_Type(loc: 0xf5'u),
  DEVID6: DEVICEID_DEVID6_Type(loc: 0xf6'u),
  DEVID7: DEVICEID_DEVID7_Type(loc: 0xf7'u),
  DEVID8: DEVICEID_DEVID8_Type(loc: 0xf8'u),
)

const EEPROM* = EEPROM_Type(
  EECR: EEPROM_EECR_Type(loc: 0x3f'u),
  EEDR: EEPROM_EEDR_Type(loc: 0x40'u),
  EEARL: EEPROM_EEARL_Type(loc: 0x41'u),
)
...
proc write*(reg: CPU_MCUCR_Type, PUD: bool = false, BODSE: bool = false, BODS: bool = false) =
  var x: uint8
...
proc read*(reg: DEVICEID_DEVID0_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID0_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID0_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID1_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID1_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID1_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID2_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID2_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID2_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID3_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID3_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID3_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID4_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID4_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID4_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID5_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID5_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID5_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID6_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID6_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID6_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID7_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID7_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID7_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: DEVICEID_DEVID8_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: DEVICEID_DEVID8_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: DEVICEID_DEVID8_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc read*(reg: EEPROM_EEARL_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: EEPROM_EEARL_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: EEPROM_EEARL_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: FUSE_EXTENDED_Type, SELFPRGEN: bool = false) =
  var x: uint8
  x.setMask((SELFPRGEN.uint8 shl 0).masked(0 .. 0))
...
func SELFPRGEN*(r: FUSE_EXTENDED_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SELFPRGEN=`*(r: var FUSE_EXTENDED_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
