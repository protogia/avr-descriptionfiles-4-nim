# Peripheral access API for ATMEGA8 microcontrollers (generated using svd2nim)
...
# #### Device Peripheral Interrupts ############################################
  ...
  irqTIMER2_COMP       =    3 # Timer/Counter2 Compare Match
  irqTIMER2_OVF        =    4 # Timer/Counter2 Overflow
  irqTIMER1_CAPT       =    5 # Timer/Counter1 Capture Event
  irqTIMER1_COMPA      =    6 # Timer/Counter1 Compare Match A
  irqTIMER1_COMPB      =    7 # Timer/Counter1 Compare Match B
  irqTIMER1_OVF        =    8 # Timer/Counter1 Overflow
  irqTIMER0_OVF        =    9 # Timer/Counter0 Overflow
  irqSPI_STC           =   10 # Serial Transfer Complete
  irqUSART_RXC         =   11 # USART, Rx Complete
  irqUSART_UDRE        =   12 # USART Data Register Empty
  irqUSART_TXC         =   13 # USART, Tx Complete
  irqADC               =   14 # ADC Conversion Complete
  irqEE_RDY            =   15 # EEPROM Ready
  irqANA_COMP          =   16 # Analog Comparator
  irqTWI               =   17 # 2-wire Serial Interface
  irqSPM_RDY           =   18 # Store Program Memory Ready
...
const TC2* = TC2_Type(
  ...
  SFIOR: TC2_SFIOR_Type(loc: 0x50'u),
...
proc write*(reg: AC_ACSR_Type, ACIS: AC_ACSR_ACIS = VAL_0x00, ACIC: bool = false, ACIE: bool = false, ACI: bool = false, ACO: bool = false, ACBG: bool = false, ACD: bool = false) =
  var x: uint8
...
proc write*(reg: ADC_ADCSRA_Type, ADPS: ADC_ADCSRA_ADPS = VAL_0x00, ADIE: bool = false, ADIF: bool = false, ADFR: bool = false, ADSC: bool = false, ADEN: bool = false) =
  var x: uint8
  x.setMask((ADPS.uint8 shl 0).masked(0 .. 2))
  x.setMask((ADIE.uint8 shl 3).masked(3 .. 3))
  x.setMask((ADIF.uint8 shl 4).masked(4 .. 4))
  x.setMask((ADFR.uint8 shl 5).masked(5 .. 5))
...
proc write*(reg: ADC_ADMUX_Type, MUX: uint8 = 0, ADLAR: bool = false, REFS: ADC_ADMUX_REFS = VAL_0x00) =
  var x: uint8
...
func ADFR*(r: ADC_ADCSRA_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `ADFR=`*(r: var ADC_ADCSRA_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.ADC_ADCSRA_Fields
...
proc write*(reg: CPU_MCUCR_Type, ISC0: CPU_MCUCR_ISC0 = VAL_0x00, ISC1: CPU_MCUCR_ISC1 = VAL_0x00, SM: CPU_MCUCR_SM = smIDLE, SE: bool = false) =
  var x: uint8
...
proc write*(reg: CPU_MCUCSR_Type, PORF: bool = false, EXTRF: bool = false, BORF: bool = false, WDRF: bool = false) =
  var x: uint8
...
proc write*(reg: CPU_SFIOR_Type, PSR10: bool = false, PUD: bool = false, ADHSM: bool = false) =
  var x: uint8
  x.setMask((PSR10.uint8 shl 0).masked(0 .. 0))
  x.setMask((PUD.uint8 shl 2).masked(2 .. 2))
  x.setMask((ADHSM.uint8 shl 4).masked(4 .. 4))
...
func ADHSM*(r: CPU_SFIOR_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `ADHSM=`*(r: var CPU_SFIOR_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.CPU_SFIOR_Fields
...
proc write*(reg: EXINT_GICR_Type, IVCE: bool = false, IVSEL: bool = false, INT: uint8 = 0) =
  var x: uint8
  x.setMask((IVCE.uint8 shl 0).masked(0 .. 0))
  x.setMask((IVSEL.uint8 shl 1).masked(1 .. 1))
  x.setMask((INT shl 6).masked(6 .. 7))
...
proc write*(reg: EXINT_GIFR_Type, INTF: uint8 = 0) =
  var x: uint8
...
proc write*(reg: EXINT_MCUCR_Type, ISC0: EXINT_MCUCR_ISC0 = VAL_0x00, ISC1: EXINT_MCUCR_ISC1 = VAL_0x00) =
  var x: uint8
  x.setMask((ISC0.uint8 shl 0).masked(0 .. 1))
  x.setMask((ISC1.uint8 shl 2).masked(2 .. 3))
...
func INT*(r: EXINT_GICR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(6 .. 7)

proc `INT=`*(r: var EXINT_GICR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
...
proc write*(reg: FUSE_HIGH_Type, BOOTRST: bool = false, BOOTSZ: FUSE_HIGH_BOOTSZ = x1024W_0C00, EESAVE: bool = false, CKOPT: bool = false, SPIEN: bool = false, WDTON: bool = false, RSTDISBL: bool = false) =
  var x: uint8
...
proc write*(reg: SPI_SPCR_Type, SPR: SPI_SPCR_SPR = VAL_0x00, CPHA: bool = false, CPOL: bool = false, MSTR: bool = false, DORD: bool = false, SPE: bool = false, SPIE: bool = false) =
  var x: uint8
...
type TC0_TCCR0_CS00* {.size: 1.} = enum
  VAL_0x00 = 0x0,
  VAL_0x01 = 0x1,
...
proc write*(reg: TC0_TCCR0_Type, CS00: TC0_TCCR0_CS00 = VAL_0x00, CS01: bool = false, CS02: bool = false) =
  var x: uint8
...
proc write*(reg: TC0_TIFR_Type, TOV0: bool = false) =
  var x: uint8
...
proc write*(reg: TC0_TIMSK_Type, TOIE0: bool = false) =
  var x: uint8
...
func CS00*(r: TC0_TCCR0_Fields): UncheckedEnum[TC0_TCCR0_CS00] {.inline.} =
  toUncheckedEnum[TC0_TCCR0_CS00](r.uint8.bitsliced(0 .. 0).int)

proc `CS00=`*(r: var TC0_TCCR0_Fields, val: TC0_TCCR0_CS00) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.TC0_TCCR0_Fields

func CS01*(r: TC0_TCCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `CS01=`*(r: var TC0_TCCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.TC0_TCCR0_Fields

func CS02*(r: TC0_TCCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `CS02=`*(r: var TC0_TCCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.TC0_TCCR0_Fields
...
proc write*(reg: TC1_TCCR1B_Type, CS1: TC1_TCCR1B_CS1 = VAL_0x00, WGM1: uint8 = 0, ICES1: bool = false, ICNC1: bool = false) =
  var x: uint8
...
type
  ...
  TC2_SFIOR_Fields* = distinct uint8
...
proc read*(reg: TC2_SFIOR_Type): TC2_SFIOR_Fields {.inline.} =
  volatileLoad(cast[ptr TC2_SFIOR_Fields](reg.loc))

proc write*(reg: TC2_SFIOR_Type, val: TC2_SFIOR_Fields) {.inline.} =
  volatileStore(cast[ptr TC2_SFIOR_Fields](reg.loc), val)

proc write*(reg: TC2_SFIOR_Type, PSR2: bool = false) =
  var x: uint8
  x.setMask((PSR2.uint8 shl 1).masked(1 .. 1))
  reg.write x.TC2_SFIOR_Fields

template modifyIt*(reg: TC2_SFIOR_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: TC2_TCCR2_Type, CS2: TC2_TCCR2_CS2 = VAL_0x00, WGM21: bool = false, COM2: uint8 = 0, WGM20: TC2_TCCR2_WGM20 = VAL_0x00, FOC2: bool = false) =
  var x: uint8
...
func PSR2*(r: TC2_SFIOR_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `PSR2=`*(r: var TC2_SFIOR_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.TC2_SFIOR_Fields
...
proc write*(reg: TWI_TWSR_Type, TWPS: TWI_TWSR_TWPS = VAL_0x00, TWS: uint8 = 0) =
  var x: uint8
...
proc write*(reg: USART_UCSRC_Type, UCPOL: bool = false, UCSZ: uint8 = 0, USBS: USART_UCSRC_USBS = VAL_0x00, UPM: USART_UCSRC_UPM = VAL_0x00, UMSEL: USART_UCSRC_UMSEL = VAL_0x00, URSEL: bool = false) =
  var x: uint8
...
proc write*(reg: WDT_WDTCR_Type, WDP: WDT_WDTCR_WDP = VAL_0x00, WDE: bool = false, WDCE: bool = false) =
  var x: uint8
