# Peripheral access API for AT90PWM316 microcontrollers (generated using svd2nim)
...
type EUSART_MUBRRH_Type* = object
  loc: uint

type EUSART_MUBRRL_Type* = object
  loc: uint
...
type EUSART_Type* = object
  ...
  MUBRRL*: EUSART_MUBRRL_Type
  MUBRRH*: EUSART_MUBRRH_Type
...
const EUSART* = EUSART_Type(
  ...
  MUBRRL: EUSART_MUBRRL_Type(loc: 0xcc'u),
  MUBRRH: EUSART_MUBRRH_Type(loc: 0xcd'u),
...
const USART* = USART_Type(
  ...
  UBRRL: USART_UBRRL_Type(loc: 0xc4'u),
  UBRRH: USART_UBRRH_Type(loc: 0xc5'u),
...
proc write*(reg: ADC_ADCSRB_Type, ADTS0: bool = false, ADTS1: bool = false, ADTS2: bool = false, ADTS3: bool = false, ADHSM: bool = false) =
  var x: uint8
...
proc read*(reg: EUSART_MUBRRH_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: EUSART_MUBRRH_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: EUSART_MUBRRH_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: EUSART_MUBRRL_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: EUSART_MUBRRL_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: EUSART_MUBRRL_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: FUSE_EXTENDED_Type, BOOTRST: bool = false, BOOTSZ: FUSE_EXTENDED_BOOTSZ = x2048W_1800, PSCRV: bool = false, PSC0RB: bool = false, PSC1RB: bool = false, PSC2RB: bool = false) =
  var x: uint8
...
proc write*(reg: USART_UBRRH_Type, UBRR: uint8 = 0) =
  var x: uint8
  x.setMask((UBRR shl 0).masked(0 .. 3))
