# Peripheral access API for ATMEGA16HVBREVB microcontrollers (generated using svd2nim)
...
proc write*(reg: FUSE_HIGH_Type, BOOTRST: bool = false, BOOTSZ: FUSE_HIGH_BOOTSZ = x2048W_1800, DWEN: bool = false, DUVRDINIT: bool = false) =
  var x: uint8
  x.setMask((BOOTRST.uint8 shl 0).masked(0 .. 0))
  x.setMask((BOOTSZ.uint8 shl 1).masked(1 .. 2))
  x.setMask((DWEN.uint8 shl 3).masked(3 .. 3))
  x.setMask((DUVRDINIT.uint8 shl 4).masked(4 .. 4))
...
func DUVRDINIT*(r: FUSE_HIGH_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `DUVRDINIT=`*(r: var FUSE_HIGH_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.FUSE_HIGH_Fields
