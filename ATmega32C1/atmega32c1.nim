# Peripheral access API for ATMEGA32C1 microcontrollers (generated using svd2nim)
...
proc write*(reg: DAC_DAC_Type, DACL: uint16 = 0, DACH: uint16 = 0) =
  var x: uint16
  x.setMask((DACL shl 0).masked(0 .. 7))
  x.setMask((DACH shl 8).masked(8 .. 15))
...
func DACL*(r: DAC_DAC_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 7)

proc `DACL=`*(r: var DAC_DAC_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 7)
