# Peripheral access API for ATMEGA3290A microcontrollers (generated using svd2nim)
...
# #### Device Peripheral Interrupts ############################################
  ...
  irqUSART_RX          =   13 # USART, Rx Complete
  ...
  irqPCINT2            =   23 # Pin Change Interrupt Request 2
  irqPCINT3            =   24 # Pin Change Interrupt Request 3
...
const LCD* = LCD_Type(
  ...
  LCDDR04: LCD_LCDDR04_Type(loc: 0xf0'u),
  ...
  LCDDR09: LCD_LCDDR09_Type(loc: 0xf5'u),
  ...
  LCDDR14: LCD_LCDDR14_Type(loc: 0xfa'u),
  ...
  LCDDR19: LCD_LCDDR19_Type(loc: 0xff'u),
)
...
const PORTH* = PORTH_Type(
  PINH: PORTH_PINH_Type(loc: 0xd8'u),
  DDRH: PORTH_DDRH_Type(loc: 0xd9'u),
  PORTH: PORTH_PORTH_Type(loc: 0xda'u),
)

const PORTJ* = PORTJ_Type(
  PINJ: PORTJ_PINJ_Type(loc: 0xdb'u),
  DDRJ: PORTJ_DDRJ_Type(loc: 0xdc'u),
  PORTJ: PORTJ_PORTJ_Type(loc: 0xdd'u),
)
...
proc write*(reg: LCD_LCDCCR_Type, LCDCC: uint8 = 0, LCDDC: uint8 = 0) =
  var x: uint8
  x.setMask((LCDCC shl 0).masked(0 .. 3))
  x.setMask((LCDDC shl 5).masked(5 .. 7))
...
proc write*(reg: LCD_LCDCRA_Type, LCDBL: bool = false, LCDIE: bool = false, LCDIF: bool = false, LCDAB: bool = false, LCDEN: bool = false) =
  var x: uint8
...
proc read*(reg: LCD_LCDDR00_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR00_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR01_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR01_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR02_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR02_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR03_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR03_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR04_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR04_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: LCD_LCDDR04_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: LCD_LCDDR05_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR05_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR06_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR06_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR07_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR07_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR08_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR08_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: LCD_LCDDR09_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR09_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: LCD_LCDDR09_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc read*(reg: LCD_LCDDR14_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR14_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: LCD_LCDDR14_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc read*(reg: LCD_LCDDR19_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: LCD_LCDDR19_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: LCD_LCDDR19_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
func LCDCC*(r: LCD_LCDCCR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 3)

proc `LCDCC=`*(r: var LCD_LCDCCR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 3)
  tmp.setMask((val shl 0).masked(0 .. 3))
  r = tmp.LCD_LCDCCR_Fields

func LCDDC*(r: LCD_LCDCCR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(5 .. 7)

proc `LCDDC=`*(r: var LCD_LCDCCR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 7)
...
proc read*(reg: PORTH_DDRH_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: PORTH_DDRH_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: PORTH_PINH_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: PORTH_PINH_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: PORTH_PORTH_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: PORTH_PORTH_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: PORTJ_DDRJ_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: PORTJ_DDRJ_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: PORTJ_PINJ_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: PORTJ_PINJ_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: PORTJ_PORTJ_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: PORTJ_PORTJ_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
