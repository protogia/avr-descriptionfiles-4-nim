# Peripheral access API for ATMEGA128 microcontrollers (generated using svd2nim)
...
proc write*(reg: CPU_XDIV_Type, XDIV0: bool = false, XDIV1: bool = false, XDIV2: bool = false, XDIV3: bool = false, XDIV4: bool = false, XDIV5: bool = false, XDIV6: bool = false, XDIVEN: bool = false) =
  var x: uint8
  x.setMask((XDIV0.uint8 shl 0).masked(0 .. 0))
  x.setMask((XDIV1.uint8 shl 1).masked(1 .. 1))
  x.setMask((XDIV2.uint8 shl 2).masked(2 .. 2))
  x.setMask((XDIV3.uint8 shl 3).masked(3 .. 3))
  x.setMask((XDIV4.uint8 shl 4).masked(4 .. 4))
  x.setMask((XDIV5.uint8 shl 5).masked(5 .. 5))
  x.setMask((XDIV6.uint8 shl 6).masked(6 .. 6))
...
func XDIV0*(r: CPU_XDIV_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `XDIV0=`*(r: var CPU_XDIV_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.CPU_XDIV_Fields

func XDIV1*(r: CPU_XDIV_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `XDIV1=`*(r: var CPU_XDIV_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.CPU_XDIV_Fields

func XDIV2*(r: CPU_XDIV_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `XDIV2=`*(r: var CPU_XDIV_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.CPU_XDIV_Fields

func XDIV3*(r: CPU_XDIV_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `XDIV3=`*(r: var CPU_XDIV_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.CPU_XDIV_Fields

func XDIV4*(r: CPU_XDIV_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `XDIV4=`*(r: var CPU_XDIV_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.CPU_XDIV_Fields

func XDIV5*(r: CPU_XDIV_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `XDIV5=`*(r: var CPU_XDIV_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.CPU_XDIV_Fields

func XDIV6*(r: CPU_XDIV_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `XDIV6=`*(r: var CPU_XDIV_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.CPU_XDIV_Fields
...
proc write*(reg: FUSE_HIGH_Type, BOOTRST: bool = false, BOOTSZ: FUSE_HIGH_BOOTSZ = x4096W_F000, EESAVE: bool = false, CKOPT: bool = false, SPIEN: bool = false, JTAGEN: bool = false, OCDEN: bool = false) =
  var x: uint8
