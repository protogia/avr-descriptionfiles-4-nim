# Peripheral access API for ATMEGA16U4 microcontrollers (generated using svd2nim)
...
# #### Device Peripheral Interrupts ############################################
  ...
  irqReserved1         =    5 # Reserved1
  irqReserved2         =    6 # Reserved2
  irqINT6              =    7 # External Interrupt Request 6
  irqReserved3         =    8 # Reserved3
  ...
  irqReserved4         =   13 # Reserved4
  irqReserved5         =   14 # Reserved5
  irqReserved6         =   15 # Reserved6
  ...
  irqTIMER4_COMPA      =   38 # Timer/Counter4 Compare Match A
  irqTIMER4_COMPB      =   39 # Timer/Counter4 Compare Match B
  irqTIMER4_COMPD      =   40 # Timer/Counter4 Compare Match D
  irqTIMER4_OVF        =   41 # Timer/Counter4 Overflow
  irqTIMER4_FPF        =   42 # Timer/Counter4 Fault Protection Interrupt
...
type PLL_PLLFRQ_Type* = object
  loc: uint

type PLL_Type* = object
  PLLCSR*: PLL_PLLCSR_Type
  PLLFRQ*: PLL_PLLFRQ_Type
...
type TC4_OCR4D_Type* = object
  loc: uint

type TC4_TC4H_Type* = object
  loc: uint
...
type TC4_TCCR4D_Type* = object
  loc: uint

type TC4_TCCR4E_Type* = object
  loc: uint
...
type TC4_Type* = object
  ...
  TCCR4D*: TC4_TCCR4D_Type
  TCCR4E*: TC4_TCCR4E_Type
  OCR4A*: TC4_OCR4A_Type
  OCR4B*: TC4_OCR4B_Type
  OCR4C*: TC4_OCR4C_Type
  OCR4D*: TC4_OCR4D_Type
  DT4*: TC4_DT4_Type
...
type USB_DEVICE_UHWCON_Type* = object
  loc: uint

type USB_DEVICE_USBCON_Type* = object
  loc: uint

type USB_DEVICE_USBINT_Type* = object
  loc: uint

type USB_DEVICE_USBSTA_Type* = object
  loc: uint

type USB_DEVICE_Type* = object
  UHWCON*: USB_DEVICE_UHWCON_Type
  USBCON*: USB_DEVICE_USBCON_Type
  USBSTA*: USB_DEVICE_USBSTA_Type
  USBINT*: USB_DEVICE_USBINT_Type
...
const CPU* = CPU_Type(
  ...
  RCCTRL: CPU_RCCTRL_Type(loc: 0x67'u),
  CLKSEL0: CPU_CLKSEL0_Type(loc: 0xc5'u),
  CLKSEL1: CPU_CLKSEL1_Type(loc: 0xc6'u),
  CLKSTA: CPU_CLKSTA_Type(loc: 0xc7'u),
)
...
const PLL* = PLL_Type(
  ...
  PLLFRQ: PLL_PLLFRQ_Type(loc: 0x52'u),
)
...
const TC4* = TC4_Type(
  ...
  TCNT4: TC4_TCNT4_Type(loc: 0xbe'u),
  TC4H: TC4_TC4H_Type(loc: 0xbf'u),
  TCCR4A: TC4_TCCR4A_Type(loc: 0xc0'u),
  TCCR4B: TC4_TCCR4B_Type(loc: 0xc1'u),
  TCCR4C: TC4_TCCR4C_Type(loc: 0xc2'u),
  TCCR4D: TC4_TCCR4D_Type(loc: 0xc3'u),
  TCCR4E: TC4_TCCR4E_Type(loc: 0xc4'u),
  OCR4A: TC4_OCR4A_Type(loc: 0xcf'u),
  OCR4B: TC4_OCR4B_Type(loc: 0xd0'u),
  OCR4C: TC4_OCR4C_Type(loc: 0xd1'u),
  OCR4D: TC4_OCR4D_Type(loc: 0xd2'u),
  DT4: TC4_DT4_Type(loc: 0xd4'u),
)
...
const USB_DEVICE* = USB_DEVICE_Type(
  UHWCON: USB_DEVICE_UHWCON_Type(loc: 0xd7'u),
  USBCON: USB_DEVICE_USBCON_Type(loc: 0xd8'u),
  USBSTA: USB_DEVICE_USBSTA_Type(loc: 0xd9'u),
  USBINT: USB_DEVICE_USBINT_Type(loc: 0xda'u),
...
proc write*(reg: ADC_ADC_Type, ADC: uint16 = 0) =
  var x: uint16
  x.setMask((ADC shl 0).masked(0 .. 9))
...
proc write*(reg: ADC_ADCSRB_Type, ADTS: ADC_ADCSRB_ADTS = FREE_RUNNING_MODE, MUX5: bool = false, ADHSM: bool = false) =
  var x: uint8
  x.setMask((ADTS.uint8 shl 0).masked(0 .. 3))
  x.setMask((MUX5.uint8 shl 5).masked(5 .. 5))
...
proc write*(reg: ADC_DIDR2_Type, ADC8D: bool = false, ADC9D: bool = false, ADC10D: bool = false, ADC11D: bool = false, ADC12D: bool = false, ADC13D: bool = false) =
  var x: uint8
...
func ADCField*(r: ADC_ADC_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 9)

proc `ADCField=`*(r: var ADC_ADC_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 9)
...
type
  ...
  CPU_RCCTRL_Fields* = distinct uint8
...
type CPU_RAMPZ_RAMPZ* {.size: 1.} = enum
...
proc write*(reg: CPU_PRR1_Type, PRUSART1: bool = false, PRTIM3: bool = false, PRTIM4: bool = false, PRUSB: bool = false) =
  var x: uint8
...
proc write*(reg: CPU_RAMPZ_Type, RAMPZ: CPU_RAMPZ_RAMPZ = DEFAULT_VALUE_OF_Z_POINTER_MSB_S) =
  var x: uint8
  x.setMask((RAMPZ.uint8 shl 0).masked(0 .. 1))
...
proc read*(reg: CPU_RCCTRL_Type): CPU_RCCTRL_Fields {.inline.} =
  volatileLoad(cast[ptr CPU_RCCTRL_Fields](reg.loc))

proc write*(reg: CPU_RCCTRL_Type, val: CPU_RCCTRL_Fields) {.inline.} =
  volatileStore(cast[ptr CPU_RCCTRL_Fields](reg.loc), val)

proc write*(reg: CPU_RCCTRL_Type, RCFREQ: bool = false) =
  var x: uint8
  x.setMask((RCFREQ.uint8 shl 0).masked(0 .. 0))
  reg.write x.CPU_RCCTRL_Fields

template modifyIt*(reg: CPU_RCCTRL_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
func RAMPZ*(r: CPU_RAMPZ_Fields): UncheckedEnum[CPU_RAMPZ_RAMPZ] {.inline.} =
  toUncheckedEnum[CPU_RAMPZ_RAMPZ](r.uint8.bitsliced(0 .. 1).int)

proc `RAMPZ=`*(r: var CPU_RAMPZ_Fields, val: CPU_RAMPZ_RAMPZ) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 1)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 1))
  r = tmp.CPU_RAMPZ_Fields

func RCFREQ*(r: CPU_RCCTRL_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `RCFREQ=`*(r: var CPU_RCCTRL_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.CPU_RCCTRL_Fields
...
proc write*(reg: FUSE_HIGH_Type, BOOTRST: bool = false, BOOTSZ: FUSE_HIGH_BOOTSZ = x2048W_1800, EESAVE: bool = false, WDTON: bool = false, SPIEN: bool = false, JTAGEN: bool = false, OCDEN: bool = false) =
  var x: uint8
...
type
  ...
  PLL_PLLFRQ_Fields* = distinct uint8
...
proc write*(reg: PLL_PLLCSR_Type, PLOCK: bool = false, PLLE: bool = false, PINDIV: bool = false) =
  var x: uint8
  x.setMask((PLOCK.uint8 shl 0).masked(0 .. 0))
  x.setMask((PLLE.uint8 shl 1).masked(1 .. 1))
  x.setMask((PINDIV.uint8 shl 4).masked(4 .. 4))
...
proc read*(reg: PLL_PLLFRQ_Type): PLL_PLLFRQ_Fields {.inline.} =
  volatileLoad(cast[ptr PLL_PLLFRQ_Fields](reg.loc))

proc write*(reg: PLL_PLLFRQ_Type, val: PLL_PLLFRQ_Fields) {.inline.} =
  volatileStore(cast[ptr PLL_PLLFRQ_Fields](reg.loc), val)

proc write*(reg: PLL_PLLFRQ_Type, PDIV: uint8 = 0, PLLTM: uint8 = 0, PLLUSB: bool = false, PINMUX: bool = false) =
  var x: uint8
  x.setMask((PDIV shl 0).masked(0 .. 3))
  x.setMask((PLLTM shl 4).masked(4 .. 5))
  x.setMask((PLLUSB.uint8 shl 6).masked(6 .. 6))
  x.setMask((PINMUX.uint8 shl 7).masked(7 .. 7))
  reg.write x.PLL_PLLFRQ_Fields

template modifyIt*(reg: PLL_PLLFRQ_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
func PINDIV*(r: PLL_PLLCSR_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `PINDIV=`*(r: var PLL_PLLCSR_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.PLL_PLLCSR_Fields

func PDIV*(r: PLL_PLLFRQ_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 3)

proc `PDIV=`*(r: var PLL_PLLFRQ_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 3)
  tmp.setMask((val shl 0).masked(0 .. 3))
  r = tmp.PLL_PLLFRQ_Fields

func PLLTM*(r: PLL_PLLFRQ_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 5)

proc `PLLTM=`*(r: var PLL_PLLFRQ_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 5)
  tmp.setMask((val shl 4).masked(4 .. 5))
  r = tmp.PLL_PLLFRQ_Fields

func PLLUSB*(r: PLL_PLLFRQ_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `PLLUSB=`*(r: var PLL_PLLFRQ_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.PLL_PLLFRQ_Fields

func PINMUX*(r: PLL_PLLFRQ_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `PINMUX=`*(r: var PLL_PLLFRQ_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.PLL_PLLFRQ_Fields
...
proc write*(reg: PORTC_DDRC_Type, PC6: bool = false, PC7: bool = false) =
  var x: uint8
...
proc write*(reg: PORTC_PINC_Type, PC6: bool = false, PC7: bool = false) =
  var x: uint8
  x.setMask((PC6.uint8 shl 6).masked(6 .. 6))
  x.setMask((PC7.uint8 shl 7).masked(7 .. 7))
...
proc write*(reg: PORTC_PORTC_Type, PC6: bool = false, PC7: bool = false) =
  var x: uint8
  x.setMask((PC6.uint8 shl 6).masked(6 .. 6))
  x.setMask((PC7.uint8 shl 7).masked(7 .. 7))
...
proc write*(reg: PORTE_DDRE_Type, PE2: bool = false, PE6: bool = false) =
  var x: uint8
...
proc write*(reg: PORTE_PINE_Type, PE2: bool = false, PE6: bool = false) =
  var x: uint8
  x.setMask((PE2.uint8 shl 2).masked(2 .. 2))
  x.setMask((PE6.uint8 shl 6).masked(6 .. 6))
...
proc write*(reg: PORTE_PORTE_Type, PE2: bool = false, PE6: bool = false) =
  var x: uint8
  x.setMask((PE2.uint8 shl 2).masked(2 .. 2))
  x.setMask((PE6.uint8 shl 6).masked(6 .. 6))
...
proc write*(reg: PORTF_DDRF_Type, PF0: bool = false, PF1: bool = false, PF4: bool = false, PF5: bool = false, PF6: bool = false) =
  var x: uint8
...
proc write*(reg: PORTF_PINF_Type, PF0: bool = false, PF1: bool = false, PF4: bool = false, PF5: bool = false, PF6: bool = false) =
  var x: uint8
  x.setMask((PF0.uint8 shl 0).masked(0 .. 0))
  x.setMask((PF1.uint8 shl 1).masked(1 .. 1))
  x.setMask((PF4.uint8 shl 4).masked(4 .. 4))
...
proc write*(reg: PORTF_PORTF_Type, PF0: bool = false, PF1: bool = false, PF4: bool = false, PF5: bool = false, PF6: bool = false) =
  var x: uint8
  x.setMask((PF0.uint8 shl 0).masked(0 .. 0))
  x.setMask((PF1.uint8 shl 1).masked(1 .. 1))
  x.setMask((PF4.uint8 shl 4).masked(4 .. 4))
...
type
  TC4_TC4H_Fields* = distinct uint8
  TC4_TCCR4A_Fields* = distinct uint8
  TC4_TCCR4B_Fields* = distinct uint8
  TC4_TCCR4C_Fields* = distinct uint8
  TC4_TCCR4D_Fields* = distinct uint8
  TC4_TCCR4E_Fields* = distinct uint8
  TC4_TIFR4_Fields* = distinct uint8
  TC4_TIMSK4_Fields* = distinct uint8

proc read*(reg: TC4_DT4_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TC4_DT4_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: TC4_DT4_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: TC4_OCR4A_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TC4_OCR4A_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: TC4_OCR4B_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TC4_OCR4B_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: TC4_OCR4C_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TC4_OCR4C_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc read*(reg: TC4_OCR4D_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TC4_OCR4D_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: TC4_OCR4D_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: TC4_TC4H_Type): TC4_TC4H_Fields {.inline.} =
  volatileLoad(cast[ptr TC4_TC4H_Fields](reg.loc))

proc write*(reg: TC4_TC4H_Type, val: TC4_TC4H_Fields) {.inline.} =
  volatileStore(cast[ptr TC4_TC4H_Fields](reg.loc), val)

proc write*(reg: TC4_TC4H_Type, TC4: uint8 = 0) =
  var x: uint8
  x.setMask((TC4 shl 0).masked(0 .. 2))
  reg.write x.TC4_TC4H_Fields

template modifyIt*(reg: TC4_TC4H_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: TC4_TCCR4A_Type, PWM4B: bool = false, PWM4A: bool = false, FOC4B: bool = false, FOC4A: bool = false, COM4B: uint8 = 0, COM4A: uint8 = 0) =
  var x: uint8
  x.setMask((PWM4B.uint8 shl 0).masked(0 .. 0))
  x.setMask((PWM4A.uint8 shl 1).masked(1 .. 1))
  x.setMask((FOC4B.uint8 shl 2).masked(2 .. 2))
  x.setMask((FOC4A.uint8 shl 3).masked(3 .. 3))
...
proc write*(reg: TC4_TCCR4B_Type, CS4: uint8 = 0, DTPS4: uint8 = 0, PSR4: bool = false, PWM4X: bool = false) =
  var x: uint8
  x.setMask((CS4 shl 0).masked(0 .. 3))
  x.setMask((DTPS4 shl 4).masked(4 .. 5))
  x.setMask((PSR4.uint8 shl 6).masked(6 .. 6))
  x.setMask((PWM4X.uint8 shl 7).masked(7 .. 7))
...
proc write*(reg: TC4_TCCR4C_Type, PWM4D: bool = false, FOC4D: bool = false, COM4D: uint8 = 0, COM4B0S: bool = false, COM4B1S: bool = false, COM4A0S: bool = false, COM4A1S: bool = false) =
  var x: uint8
  x.setMask((PWM4D.uint8 shl 0).masked(0 .. 0))
  x.setMask((FOC4D.uint8 shl 1).masked(1 .. 1))
  x.setMask((COM4D shl 2).masked(2 .. 3))
  x.setMask((COM4B0S.uint8 shl 4).masked(4 .. 4))
  x.setMask((COM4B1S.uint8 shl 5).masked(5 .. 5))
  x.setMask((COM4A0S.uint8 shl 6).masked(6 .. 6))
  x.setMask((COM4A1S.uint8 shl 7).masked(7 .. 7))
...
proc read*(reg: TC4_TCCR4D_Type): TC4_TCCR4D_Fields {.inline.} =
  volatileLoad(cast[ptr TC4_TCCR4D_Fields](reg.loc))

proc write*(reg: TC4_TCCR4D_Type, val: TC4_TCCR4D_Fields) {.inline.} =
  volatileStore(cast[ptr TC4_TCCR4D_Fields](reg.loc), val)

proc write*(reg: TC4_TCCR4D_Type, WGM4: uint8 = 0, FPF4: bool = false, FPAC4: bool = false, FPES4: bool = false, FPNC4: bool = false, FPEN4: bool = false, FPIE4: bool = false) =
  var x: uint8
  x.setMask((WGM4 shl 0).masked(0 .. 1))
  x.setMask((FPF4.uint8 shl 2).masked(2 .. 2))
  x.setMask((FPAC4.uint8 shl 3).masked(3 .. 3))
  x.setMask((FPES4.uint8 shl 4).masked(4 .. 4))
  x.setMask((FPNC4.uint8 shl 5).masked(5 .. 5))
  x.setMask((FPEN4.uint8 shl 6).masked(6 .. 6))
  x.setMask((FPIE4.uint8 shl 7).masked(7 .. 7))
  reg.write x.TC4_TCCR4D_Fields

template modifyIt*(reg: TC4_TCCR4D_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: TC4_TCCR4E_Type): TC4_TCCR4E_Fields {.inline.} =
  volatileLoad(cast[ptr TC4_TCCR4E_Fields](reg.loc))

proc write*(reg: TC4_TCCR4E_Type, val: TC4_TCCR4E_Fields) {.inline.} =
  volatileStore(cast[ptr TC4_TCCR4E_Fields](reg.loc), val)

proc write*(reg: TC4_TCCR4E_Type, OC4OE: uint8 = 0, ENHC4: bool = false, TLOCK4: bool = false) =
  var x: uint8
  x.setMask((OC4OE shl 0).masked(0 .. 5))
  x.setMask((ENHC4.uint8 shl 6).masked(6 .. 6))
  x.setMask((TLOCK4.uint8 shl 7).masked(7 .. 7))
  reg.write x.TC4_TCCR4E_Fields

template modifyIt*(reg: TC4_TCCR4E_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: TC4_TCNT4_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TC4_TCNT4_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc write*(reg: TC4_TIFR4_Type, TOV4: bool = false, OCF4B: bool = false, OCF4A: bool = false, OCF4D: bool = false) =
  var x: uint8
  x.setMask((TOV4.uint8 shl 2).masked(2 .. 2))
  x.setMask((OCF4B.uint8 shl 5).masked(5 .. 5))
  x.setMask((OCF4A.uint8 shl 6).masked(6 .. 6))
  x.setMask((OCF4D.uint8 shl 7).masked(7 .. 7))
...
proc write*(reg: TC4_TIMSK4_Type, TOIE4: bool = false, OCIE4B: bool = false, OCIE4A: bool = false, OCIE4D: bool = false) =
  var x: uint8
  x.setMask((TOIE4.uint8 shl 2).masked(2 .. 2))
  x.setMask((OCIE4B.uint8 shl 5).masked(5 .. 5))
  x.setMask((OCIE4A.uint8 shl 6).masked(6 .. 6))
  x.setMask((OCIE4D.uint8 shl 7).masked(7 .. 7))
...
func TC4Field*(r: TC4_TC4H_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 2)

proc `TC4Field=`*(r: var TC4_TC4H_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 2)
  tmp.setMask((val shl 0).masked(0 .. 2))
  r = tmp.TC4_TC4H_Fields

func PWM4B*(r: TC4_TCCR4A_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `PWM4B=`*(r: var TC4_TCCR4A_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.TC4_TCCR4A_Fields

func PWM4A*(r: TC4_TCCR4A_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `PWM4A=`*(r: var TC4_TCCR4A_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.TC4_TCCR4A_Fields

func FOC4B*(r: TC4_TCCR4A_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `FOC4B=`*(r: var TC4_TCCR4A_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.TC4_TCCR4A_Fields

func FOC4A*(r: TC4_TCCR4A_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `FOC4A=`*(r: var TC4_TCCR4A_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.TC4_TCCR4A_Fields
...
func CS4*(r: TC4_TCCR4B_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 3)

proc `CS4=`*(r: var TC4_TCCR4B_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 3)
  tmp.setMask((val shl 0).masked(0 .. 3))
  r = tmp.TC4_TCCR4B_Fields

func DTPS4*(r: TC4_TCCR4B_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 5)

proc `DTPS4=`*(r: var TC4_TCCR4B_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 5)
  tmp.setMask((val shl 4).masked(4 .. 5))
  r = tmp.TC4_TCCR4B_Fields

func PSR4*(r: TC4_TCCR4B_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `PSR4=`*(r: var TC4_TCCR4B_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.TC4_TCCR4B_Fields

func PWM4X*(r: TC4_TCCR4B_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `PWM4X=`*(r: var TC4_TCCR4B_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.TC4_TCCR4B_Fields

func PWM4D*(r: TC4_TCCR4C_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `PWM4D=`*(r: var TC4_TCCR4C_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.TC4_TCCR4C_Fields

func FOC4D*(r: TC4_TCCR4C_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `FOC4D=`*(r: var TC4_TCCR4C_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.TC4_TCCR4C_Fields

func COM4D*(r: TC4_TCCR4C_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(2 .. 3)

proc `COM4D=`*(r: var TC4_TCCR4C_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 3)
  tmp.setMask((val shl 2).masked(2 .. 3))
  r = tmp.TC4_TCCR4C_Fields

func COM4B0S*(r: TC4_TCCR4C_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `COM4B0S=`*(r: var TC4_TCCR4C_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.TC4_TCCR4C_Fields

func COM4B1S*(r: TC4_TCCR4C_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `COM4B1S=`*(r: var TC4_TCCR4C_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.TC4_TCCR4C_Fields

func COM4A0S*(r: TC4_TCCR4C_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `COM4A0S=`*(r: var TC4_TCCR4C_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.TC4_TCCR4C_Fields

func COM4A1S*(r: TC4_TCCR4C_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `COM4A1S=`*(r: var TC4_TCCR4C_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.TC4_TCCR4C_Fields

func WGM4*(r: TC4_TCCR4D_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 1)

proc `WGM4=`*(r: var TC4_TCCR4D_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 1)
  tmp.setMask((val shl 0).masked(0 .. 1))
  r = tmp.TC4_TCCR4D_Fields

func FPF4*(r: TC4_TCCR4D_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `FPF4=`*(r: var TC4_TCCR4D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.TC4_TCCR4D_Fields

func FPAC4*(r: TC4_TCCR4D_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `FPAC4=`*(r: var TC4_TCCR4D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.TC4_TCCR4D_Fields

func FPES4*(r: TC4_TCCR4D_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `FPES4=`*(r: var TC4_TCCR4D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.TC4_TCCR4D_Fields

func FPNC4*(r: TC4_TCCR4D_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `FPNC4=`*(r: var TC4_TCCR4D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.TC4_TCCR4D_Fields

func FPEN4*(r: TC4_TCCR4D_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `FPEN4=`*(r: var TC4_TCCR4D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.TC4_TCCR4D_Fields

func FPIE4*(r: TC4_TCCR4D_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `FPIE4=`*(r: var TC4_TCCR4D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.TC4_TCCR4D_Fields

func OC4OE*(r: TC4_TCCR4E_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 5)

proc `OC4OE=`*(r: var TC4_TCCR4E_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 5)
  tmp.setMask((val shl 0).masked(0 .. 5))
  r = tmp.TC4_TCCR4E_Fields

func ENHC4*(r: TC4_TCCR4E_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `ENHC4=`*(r: var TC4_TCCR4E_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.TC4_TCCR4E_Fields

func TLOCK4*(r: TC4_TCCR4E_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `TLOCK4=`*(r: var TC4_TCCR4E_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.TC4_TCCR4E_Fields
...
func OCF4D*(r: TC4_TIFR4_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `OCF4D=`*(r: var TC4_TIFR4_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.TC4_TIFR4_Fields
...
func OCIE4D*(r: TC4_TIMSK4_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `OCIE4D=`*(r: var TC4_TIMSK4_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.TC4_TIMSK4_Fields
...
type
  ...
  USB_DEVICE_UEBCHX_Fields* = distinct uint8
  ...
  USB_DEVICE_UHWCON_Fields* = distinct uint8
  USB_DEVICE_USBCON_Fields* = distinct uint8
  USB_DEVICE_USBINT_Fields* = distinct uint8
  USB_DEVICE_USBSTA_Fields* = distinct uint8
...
proc write*(reg: USB_DEVICE_UDCON_Type, DETACH: bool = false, RMWKUP: bool = false, LSM: bool = false, RSTCPU: bool = false) =
  var x: uint8
  x.setMask((DETACH.uint8 shl 0).masked(0 .. 0))
  x.setMask((RMWKUP.uint8 shl 1).masked(1 .. 1))
  x.setMask((LSM.uint8 shl 2).masked(2 .. 2))
  x.setMask((RSTCPU.uint8 shl 3).masked(3 .. 3))
...
proc read*(reg: USB_DEVICE_UEBCHX_Type): USB_DEVICE_UEBCHX_Fields {.inline.} =
  volatileLoad(cast[ptr USB_DEVICE_UEBCHX_Fields](reg.loc))

proc write*(reg: USB_DEVICE_UEBCHX_Type, val: USB_DEVICE_UEBCHX_Fields) {.inline.} =
  volatileStore(cast[ptr USB_DEVICE_UEBCHX_Fields](reg.loc), val)

proc write*(reg: USB_DEVICE_UEBCHX_Type, BYCT: uint8 = 0) =
  var x: uint8
  x.setMask((BYCT shl 0).masked(0 .. 2))
  reg.write x.USB_DEVICE_UEBCHX_Fields
...
proc write*(reg: USB_DEVICE_UEINT_Type, EPINT: uint8 = 0) =
  var x: uint8
  x.setMask((EPINT shl 0).masked(0 .. 6))
...
proc write*(reg: USB_DEVICE_UENUM_Type, UENUM: uint8 = 0) =
  var x: uint8
  x.setMask((UENUM shl 0).masked(0 .. 2))
...
proc read*(reg: USB_DEVICE_UHWCON_Type): USB_DEVICE_UHWCON_Fields {.inline.} =
  volatileLoad(cast[ptr USB_DEVICE_UHWCON_Fields](reg.loc))

proc write*(reg: USB_DEVICE_UHWCON_Type, val: USB_DEVICE_UHWCON_Fields) {.inline.} =
  volatileStore(cast[ptr USB_DEVICE_UHWCON_Fields](reg.loc), val)

proc write*(reg: USB_DEVICE_UHWCON_Type, UVREGE: bool = false) =
  var x: uint8
  x.setMask((UVREGE.uint8 shl 0).masked(0 .. 0))
  reg.write x.USB_DEVICE_UHWCON_Fields

template modifyIt*(reg: USB_DEVICE_UHWCON_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: USB_DEVICE_USBCON_Type, VBUSTE: bool = false, OTGPADE: bool = false, FRZCLK: bool = false, USBE: bool = false) =
  var x: uint8
...
proc read*(reg: USB_DEVICE_USBINT_Type): USB_DEVICE_USBINT_Fields {.inline.} =
  volatileLoad(cast[ptr USB_DEVICE_USBINT_Fields](reg.loc))

proc write*(reg: USB_DEVICE_USBINT_Type, val: USB_DEVICE_USBINT_Fields) {.inline.} =
  volatileStore(cast[ptr USB_DEVICE_USBINT_Fields](reg.loc), val)

proc write*(reg: USB_DEVICE_USBINT_Type, VBUSTI: bool = false) =
  var x: uint8
  x.setMask((VBUSTI.uint8 shl 0).masked(0 .. 0))
  reg.write x.USB_DEVICE_USBINT_Fields

template modifyIt*(reg: USB_DEVICE_USBINT_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: USB_DEVICE_USBSTA_Type): USB_DEVICE_USBSTA_Fields {.inline.} =
  volatileLoad(cast[ptr USB_DEVICE_USBSTA_Fields](reg.loc))

proc write*(reg: USB_DEVICE_USBSTA_Type, val: USB_DEVICE_USBSTA_Fields) {.inline.} =
  volatileStore(cast[ptr USB_DEVICE_USBSTA_Fields](reg.loc), val)

proc write*(reg: USB_DEVICE_USBSTA_Type, VBUS: bool = false, SPEED: bool = false) =
  var x: uint8
  x.setMask((VBUS.uint8 shl 0).masked(0 .. 0))
  x.setMask((SPEED.uint8 shl 3).masked(3 .. 3))
  reg.write x.USB_DEVICE_USBSTA_Fields

template modifyIt*(reg: USB_DEVICE_USBSTA_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
func BYCT*(r: USB_DEVICE_UEBCHX_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 2)

proc `BYCT=`*(r: var USB_DEVICE_UEBCHX_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 2)
  tmp.setMask((val shl 0).masked(0 .. 2))
  r = tmp.USB_DEVICE_UEBCHX_Fields
...
func UENUM*(r: USB_DEVICE_UENUM_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 2)

proc `UENUM=`*(r: var USB_DEVICE_UENUM_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 2)
  tmp.setMask((val shl 0).masked(0 .. 2))
...
func UVREGE*(r: USB_DEVICE_UHWCON_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `UVREGE=`*(r: var USB_DEVICE_UHWCON_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.USB_DEVICE_UHWCON_Fields

func VBUSTE*(r: USB_DEVICE_USBCON_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `VBUSTE=`*(r: var USB_DEVICE_USBCON_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.USB_DEVICE_USBCON_Fields

func OTGPADE*(r: USB_DEVICE_USBCON_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `OTGPADE=`*(r: var USB_DEVICE_USBCON_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.USB_DEVICE_USBCON_Fields
...
func VBUSTI*(r: USB_DEVICE_USBINT_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `VBUSTI=`*(r: var USB_DEVICE_USBINT_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.USB_DEVICE_USBINT_Fields

func VBUS*(r: USB_DEVICE_USBSTA_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `VBUS=`*(r: var USB_DEVICE_USBSTA_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.USB_DEVICE_USBSTA_Fields

func SPEED*(r: USB_DEVICE_USBSTA_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SPEED=`*(r: var USB_DEVICE_USBSTA_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.USB_DEVICE_USBSTA_Fields
