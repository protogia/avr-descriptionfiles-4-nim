# Peripheral access API for ATMEGA32U4 microcontrollers (generated using svd2nim)
...
proc write*(reg: CPU_RAMPZ_Type, RAMPZ: CPU_RAMPZ_RAMPZ = DEFAULT_VALUE_OF_Z_POINTER_MSB_S, Res: uint8 = 0) =
  var x: uint8
  x.setMask((RAMPZ.uint8 shl 0).masked(0 .. 1))
  x.setMask((Res shl 2).masked(2 .. 7))
...
func Res*(r: CPU_RAMPZ_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(2 .. 7)

proc `Res=`*(r: var CPU_RAMPZ_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 7)
...
proc read*(reg: TC4_TC4H_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TC4_TC4H_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
