# Peripheral access API for ATMEGA324PA microcontrollers (generated using svd2nim)
...
# #### Device Peripheral Interrupts ############################################
  ...
  irqSPI_STC           =   19 # SPI Serial Transfer Complete
  ...
  irqTWI               =   26 # 2-wire Serial Interface
...
type SPI_SPCR0_Type* = object
  loc: uint

type SPI_SPDR_Type* = object
  loc: uint

type SPI_SPDR0_Type* = object
  loc: uint

type SPI_SPSR_Type* = object
  loc: uint

type SPI_SPSR0_Type* = object
  loc: uint

type SPI_Type* = object
  SPCR*: SPI_SPCR_Type
  SPCR0*: SPI_SPCR0_Type
  SPSR*: SPI_SPSR_Type
  SPSR0*: SPI_SPSR0_Type
  SPDR*: SPI_SPDR_Type
  SPDR0*: SPI_SPDR0_Type
...
const SPI* = SPI_Type(
  ...
  SPCR0: SPI_SPCR0_Type(loc: 0x4c'u),
  SPSR: SPI_SPSR_Type(loc: 0x4d'u),
  SPSR0: SPI_SPSR0_Type(loc: 0x4d'u),
  SPDR: SPI_SPDR_Type(loc: 0x4e'u),
  SPDR0: SPI_SPDR0_Type(loc: 0x4e'u),
)
...
proc write*(reg: CPU_PRR0_Type, PRADC: bool = false, PRUSART0: bool = false, PRSPI: bool = false, PRTIM1: bool = false, PRUSART1: bool = false, PRTIM0: bool = false, PRTIM2: bool = false, PRTWI: bool = false) =
  var x: uint8
...
type
  ...
  SPI_SPCR0_Fields* = distinct uint8
  SPI_SPSR_Fields* = distinct uint8
  SPI_SPSR0_Fields* = distinct uint8

type SPI_SPCR_SPR0* {.size: 1.} = enum
  FOSC_4 = 0x0,
  FOSC_16 = 0x1,

type SPI_SPCR0_SPR00* {.size: 1.} = enum
  FOSC_4 = 0x0,
  FOSC_16 = 0x1,
...
proc write*(reg: SPI_SPCR_Type, SPR0: SPI_SPCR_SPR0 = FOSC_4, SPR1: bool = false, CPHA: bool = false, CPOL: bool = false, MSTR: bool = false, DORD: bool = false, SPE: bool = false, SPIE: bool = false) =
  var x: uint8
  x.setMask((SPR0.uint8 shl 0).masked(0 .. 0))
  x.setMask((SPR1.uint8 shl 1).masked(1 .. 1))
...
proc read*(reg: SPI_SPCR0_Type): SPI_SPCR0_Fields {.inline.} =
  volatileLoad(cast[ptr SPI_SPCR0_Fields](reg.loc))

proc write*(reg: SPI_SPCR0_Type, val: SPI_SPCR0_Fields) {.inline.} =
  volatileStore(cast[ptr SPI_SPCR0_Fields](reg.loc), val)

proc write*(reg: SPI_SPCR0_Type, SPR00: SPI_SPCR0_SPR00 = FOSC_4, SPR10: bool = false, CPHA0: bool = false, CPOL0: bool = false, MSTR0: bool = false, DORD0: bool = false, SPE0: bool = false, SPIE0: bool = false) =
  var x: uint8
  x.setMask((SPR00.uint8 shl 0).masked(0 .. 0))
  x.setMask((SPR10.uint8 shl 1).masked(1 .. 1))
  x.setMask((CPHA0.uint8 shl 2).masked(2 .. 2))
  x.setMask((CPOL0.uint8 shl 3).masked(3 .. 3))
  x.setMask((MSTR0.uint8 shl 4).masked(4 .. 4))
  x.setMask((DORD0.uint8 shl 5).masked(5 .. 5))
  x.setMask((SPE0.uint8 shl 6).masked(6 .. 6))
  x.setMask((SPIE0.uint8 shl 7).masked(7 .. 7))
  reg.write x.SPI_SPCR0_Fields

template modifyIt*(reg: SPI_SPCR0_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc read*(reg: SPI_SPDR0_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: SPI_SPDR0_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: SPI_SPDR0_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc read*(reg: SPI_SPSR0_Type): SPI_SPSR0_Fields {.inline.} =
  volatileLoad(cast[ptr SPI_SPSR0_Fields](reg.loc))

proc write*(reg: SPI_SPSR0_Type, val: SPI_SPSR0_Fields) {.inline.} =
  volatileStore(cast[ptr SPI_SPSR0_Fields](reg.loc), val)

proc write*(reg: SPI_SPSR0_Type, SPI2X0: bool = false, WCOL0: bool = false, SPIF0: bool = false) =
  var x: uint8
  x.setMask((SPI2X0.uint8 shl 0).masked(0 .. 0))
  x.setMask((WCOL0.uint8 shl 6).masked(6 .. 6))
  x.setMask((SPIF0.uint8 shl 7).masked(7 .. 7))
  reg.write x.SPI_SPSR0_Fields

template modifyIt*(reg: SPI_SPSR0_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

func SPR0*(r: SPI_SPCR_Fields): UncheckedEnum[SPI_SPCR_SPR0] {.inline.} =
  toUncheckedEnum[SPI_SPCR_SPR0](r.uint8.bitsliced(0 .. 0).int)

proc `SPR0=`*(r: var SPI_SPCR_Fields, val: SPI_SPCR_SPR0) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.SPI_SPCR_Fields

func SPR1*(r: SPI_SPCR_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SPR1=`*(r: var SPI_SPCR_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.SPI_SPCR_Fields
...
func SPR00*(r: SPI_SPCR0_Fields): UncheckedEnum[SPI_SPCR0_SPR00] {.inline.} =
  toUncheckedEnum[SPI_SPCR0_SPR00](r.uint8.bitsliced(0 .. 0).int)

proc `SPR00=`*(r: var SPI_SPCR0_Fields, val: SPI_SPCR0_SPR00) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.SPI_SPCR0_Fields

func SPR10*(r: SPI_SPCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `SPR10=`*(r: var SPI_SPCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.SPI_SPCR0_Fields

func CPHA0*(r: SPI_SPCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `CPHA0=`*(r: var SPI_SPCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.SPI_SPCR0_Fields

func CPOL0*(r: SPI_SPCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `CPOL0=`*(r: var SPI_SPCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.SPI_SPCR0_Fields

func MSTR0*(r: SPI_SPCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `MSTR0=`*(r: var SPI_SPCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.SPI_SPCR0_Fields

func DORD0*(r: SPI_SPCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `DORD0=`*(r: var SPI_SPCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.SPI_SPCR0_Fields

func SPE0*(r: SPI_SPCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SPE0=`*(r: var SPI_SPCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.SPI_SPCR0_Fields

func SPIE0*(r: SPI_SPCR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SPIE0=`*(r: var SPI_SPCR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.SPI_SPCR0_Fields
...
func SPI2X0*(r: SPI_SPSR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SPI2X0=`*(r: var SPI_SPSR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.SPI_SPSR0_Fields

func WCOL0*(r: SPI_SPSR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `WCOL0=`*(r: var SPI_SPSR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.SPI_SPSR0_Fields

func SPIF0*(r: SPI_SPSR0_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SPIF0=`*(r: var SPI_SPSR0_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.SPI_SPSR0_Fields
...
type
  TC1_OCR1B_Fields* = distinct uint16
...
proc read*(reg: TC1_OCR1B_Type): TC1_OCR1B_Fields {.inline.} =
  volatileLoad(cast[ptr TC1_OCR1B_Fields](reg.loc))

proc write*(reg: TC1_OCR1B_Type, val: TC1_OCR1B_Fields) {.inline.} =
  volatileStore(cast[ptr TC1_OCR1B_Fields](reg.loc), val)

proc write*(reg: TC1_OCR1B_Type, OCR1B: uint16 = 0) =
  var x: uint16
  x.setMask((OCR1B shl 0).masked(0 .. 7))
  reg.write x.TC1_OCR1B_Fields
...
func OCR1B*(r: TC1_OCR1B_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 7)

proc `OCR1B=`*(r: var TC1_OCR1B_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 7)
  tmp.setMask((val shl 0).masked(0 .. 7))
  r = tmp.TC1_OCR1B_Fields
