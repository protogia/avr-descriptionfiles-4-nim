# Peripheral access API for ATMEGA32A microcontrollers (generated using svd2nim)
...
proc read*(reg: TWI_TWAR_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TWI_TWAR_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
