# Peripheral access API for AT90PWM3 microcontrollers (generated using svd2nim)
...
proc write*(reg: ADC_ADCSRB_Type, ADTS: uint8 = 0, ADASCR: bool = false, ADHSM: bool = false) =
  var x: uint8
...
proc write*(reg: PSC0_PICR0_Type, PICR0: uint16 = 0) =
  var x: uint16
...
type PSC1_PCNF1_PMODE1* {.size: 1.} = enum
  ONE_RAMP_MODE = 0x0,
  TWO_RAMP_MODE = 0x1,
  FOUR_RAMP_MODE = 0x2,
  CENTER_ALIGNED_MODE = 0x3,
...
type PSC1_PIFR1_PRN1* {.size: 1.} = enum
  x1 = 0x0,
  x2 = 0x1,
  x3 = 0x2,
  x4 = 0x3,

type PSC1_PSOC1_PSYNC1_* {.size: 1.} = enum
  SEND_SIGNAL_ON_LEADING_EDGE_OF_PSCOUTN0 = 0x0,
  SEND_SIGNAL_ON_TRAILING_EDGE_OF_PSCOUTN0 = 0x1,
  SEND_SIGNAL_ON_LEADING_EDGE_OF_PSCOUTN1 = 0x2,
  SEND_SIGNAL_ON_TRAILING_EDGE_OF_PSCOUTN1 = 0x3,

proc read*(reg: PSC1_OCR1RA_Type): uint16 {.inline.} =
  volatileLoad(cast[ptr uint16](reg.loc))

proc write*(reg: PSC1_OCR1RA_Type, val: uint16) {.inline.} =
  volatileStore(cast[ptr uint16](reg.loc), val)
...
proc read*(reg: PSC1_OCR1SA_Type): uint16 {.inline.} =
  volatileLoad(cast[ptr uint16](reg.loc))

proc write*(reg: PSC1_OCR1SA_Type, val: uint16) {.inline.} =
  volatileStore(cast[ptr uint16](reg.loc), val)
...
proc read*(reg: PSC1_OCR1SB_Type): uint16 {.inline.} =
  volatileLoad(cast[ptr uint16](reg.loc))

proc write*(reg: PSC1_OCR1SB_Type, val: uint16) {.inline.} =
  volatileStore(cast[ptr uint16](reg.loc), val)
...
proc write*(reg: PSC1_PCNF1_Type, PCLKSEL1: bool = false, POP1: bool = false, PMODE1: PSC1_PCNF1_PMODE1 = ONE_RAMP_MODE, PLOCK1: bool = false, PALOCK1: bool = false, PFIFTY1: bool = false) =
  var x: uint8
  x.setMask((PCLKSEL1.uint8 shl 1).masked(1 .. 1))
  x.setMask((POP1.uint8 shl 2).masked(2 .. 2))
  x.setMask((PMODE1.uint8 shl 3).masked(3 .. 4))
...
proc write*(reg: PSC1_PIFR1_Type, PEOP1: bool = false, PRN1: PSC1_PIFR1_PRN1 = x1, PEV1A: bool = false, PEV1B: bool = false, PSEI1: bool = false) =
  var x: uint8
  x.setMask((PEOP1.uint8 shl 0).masked(0 .. 0))
  x.setMask((PRN1.uint8 shl 1).masked(1 .. 2))
...
proc write*(reg: PSC1_PSOC1_Type, POEN1A: bool = false, POEN1B: bool = false, PSYNC1: PSC1_PSOC1_PSYNC1_ = SEND_SIGNAL_ON_LEADING_EDGE_OF_PSCOUTN0) =
  var x: uint8
...
func PMODE1*(r: PSC1_PCNF1_Fields): UncheckedEnum[PSC1_PCNF1_PMODE1] {.inline.} =
  toUncheckedEnum[PSC1_PCNF1_PMODE1](r.uint8.bitsliced(3 .. 4).int)

proc `PMODE1=`*(r: var PSC1_PCNF1_Fields, val: PSC1_PCNF1_PMODE1) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 4)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 4))
  r = tmp.PSC1_PCNF1_Fields
...
func PRN1*(r: PSC1_PIFR1_Fields): UncheckedEnum[PSC1_PIFR1_PRN1] {.inline.} =
  toUncheckedEnum[PSC1_PIFR1_PRN1](r.uint8.bitsliced(1 .. 2).int)

proc `PRN1=`*(r: var PSC1_PIFR1_Fields, val: PSC1_PIFR1_PRN1) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 2)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 2))
  r = tmp.PSC1_PIFR1_Fields
...
func PSYNC1*(r: PSC1_PSOC1_Fields): UncheckedEnum[PSC1_PSOC1_PSYNC1_] {.inline.} =
  toUncheckedEnum[PSC1_PSOC1_PSYNC1_](r.uint8.bitsliced(4 .. 5).int)

proc `PSYNC1=`*(r: var PSC1_PSOC1_Fields, val: PSC1_PSOC1_PSYNC1_) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 5)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 5))
  r = tmp.PSC1_PSOC1_Fields
...
proc write*(reg: PSC2_PICR2_Type, PICR2: uint16 = 0) =
  var x: uint16
