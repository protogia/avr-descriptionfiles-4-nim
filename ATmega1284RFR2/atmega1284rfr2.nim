# Peripheral access API for ATMEGA1284RFR2 microcontrollers (generated using svd2nim)
...
proc write*(reg: CPU_MCUCR_Type, IVCE: bool = false, IVSEL: bool = false, PUD: bool = false, Res: uint8 = 0, JTD: bool = false) =
  var x: uint8
  x.setMask((IVCE.uint8 shl 0).masked(0 .. 0))
  x.setMask((IVSEL.uint8 shl 1).masked(1 .. 1))
  x.setMask((PUD.uint8 shl 4).masked(4 .. 4))
  x.setMask((Res shl 5).masked(5 .. 6))
...
proc read*(reg: CPU_OSCCAL_Type): CPU_OSCCAL_CAL {.inline.} =
  volatileLoad(cast[ptr CPU_OSCCAL_CAL](reg.loc))

proc write*(reg: CPU_OSCCAL_Type, val: CPU_OSCCAL_CAL) {.inline.} =
  volatileStore(cast[ptr CPU_OSCCAL_CAL](reg.loc), val)
...
proc write*(reg: CPU_RAMPZ_Type, RAMPZ0: CPU_RAMPZ_RAMPZ0 = DEFAULT_VALUE_OF_Z_POINTER_MSB_S, Res: uint8 = 0) =
  var x: uint8
...
func Res*(r: CPU_MCUCR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(5 .. 6)

proc `Res=`*(r: var CPU_MCUCR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 6)
...
proc write*(reg: TRX24_PHY_TX_PWR_Type, TX_PWR: TRX24_PHY_TX_PWR_TX_PWR = x3_5_DBM, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: TRX24_RX_SYN_Type, RX_PDT_LEVEL: TRX24_RX_SYN_RX_PDT_LEVEL = RX_PDT_LEVEL_MIN, Res: uint8 = 0, RX_OVERRIDE: bool = false, RX_PDT_DIS: bool = false) =
  var x: uint8
...
proc write*(reg: TRX24_TRX_CTRL_0_Type, Res: uint8 = 0, PMU_IF_INV: bool = false, PMU_START: bool = false, PMU_EN: bool = false, Res7: bool = false) =
  var x: uint8
  x.setMask((Res shl 0).masked(0 .. 3))
...
proc write*(reg: TRX24_TRX_CTRL_1_Type, Res: uint8 = 0, PLL_TX_FLT: bool = false, TX_AUTO_CRC_ON: bool = false, IRQ_2_EXT_EN: bool = false, PA_EXT_EN: bool = false) =
  var x: uint8
  x.setMask((Res shl 0).masked(0 .. 3))
...
func Res*(r: TRX24_PHY_TX_PWR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 7)

proc `Res=`*(r: var TRX24_PHY_TX_PWR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 7)
  tmp.setMask((val shl 4).masked(4 .. 7))
  r = tmp.TRX24_PHY_TX_PWR_Fields
...
func Res*(r: TRX24_TRX_CTRL_0_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 3)

proc `Res=`*(r: var TRX24_TRX_CTRL_0_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 3)
  tmp.setMask((val shl 0).masked(0 .. 3))
