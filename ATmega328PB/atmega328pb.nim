# Peripheral access API for ATMEGA328PB microcontrollers (generated using svd2nim)
...
# #### Device Peripheral Interrupts ############################################
  ...
  irqPCINT1            =    4 # Pin Change Interrupt Request 1
  irqPCINT2            =    5 # Pin Change Interrupt Request 2
  irqWDT               =    6 # Watchdog Time-out Interrupt
  irqTIMER2_COMPA      =    7 # Timer/Counter2 Compare Match A
  irqTIMER2_COMPB      =    8 # Timer/Counter2 Compare Match B
  ...
  irqSPI0_STC          =   17 # SPI Serial Transfer Complete
  ...
  irqTWI0              =   24 # Two-wire Serial Interface
  irqSPM_Ready         =   25 # Store Program Memory Read
  irqUSART0_START      =   26 # USART0 Start frame detection
  irqPCINT3            =   27 # Pin Change Interrupt Request 3
  irqUSART1_RX         =   28 # USART1 Rx Complete
  irqUSART1_UDRE       =   29 # USART1, Data Register Empty
  irqUSART1_TX         =   30 # USART1 Tx Complete
  irqUSART1_START      =   31 # USART1 Start frame detection
  irqTIMER3_CAPT       =   32 # Timer/Counter3 Capture Event
  irqTIMER3_COMPA      =   33 # Timer/Counter3 Compare Match A
  irqTIMER3_COMPB      =   34 # Timer/Counter3 Compare Match B
  irqTIMER3_OVF        =   35 # Timer/Counter3 Overflow
  irqCFD               =   36 # Clock failure detection interrupt
  irqPTC_EOC           =   37 # PTC End of conversion
  irqPTC_WCOMP         =   38 # PTC Window comparator mode
  irqSPI1_STC          =   39 # SPI1 Serial Transfer Complete
  irqTWI1              =   40 # TWI Transfer Complete
  ...
  irqTIMER4_OVF        =   44 # Timer/Counter4 Overflow
...
const AC* = AC_Type(
  ...
  ACSRA: AC_ACSRA_Type(loc: 0x50'u),
...
proc read*(reg: AC_ACSRA_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: AC_ACSRA_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: AC_ACSRA_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: CPU_PRR1_Type, PRTIM3: bool = false, PRSPI1: bool = false, PRTIM4: bool = false, PRPTC: bool = false, PRTWI1: bool = false) =
  var x: uint8
  x.setMask((PRTIM3.uint8 shl 0).masked(0 .. 0))
  x.setMask((PRSPI1.uint8 shl 2).masked(2 .. 2))
  x.setMask((PRTIM4.uint8 shl 3).masked(3 .. 3))
  x.setMask((PRPTC.uint8 shl 4).masked(4 .. 4))
  x.setMask((PRTWI1.uint8 shl 5).masked(5 .. 5))
...
func PRSPI1*(r: CPU_PRR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `PRSPI1=`*(r: var CPU_PRR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.CPU_PRR1_Fields
...
func PRPTC*(r: CPU_PRR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `PRPTC=`*(r: var CPU_PRR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.CPU_PRR1_Fields

func PRTWI1*(r: CPU_PRR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `PRTWI1=`*(r: var CPU_PRR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.CPU_PRR1_Fields
...
proc write*(reg: FUSE_HIGH_Type, BOOTRST: bool = false, BOOTSZ: FUSE_HIGH_BOOTSZ = x2048W_3800, EESAVE: bool = false, WDTON: bool = false, SPIEN: bool = false, DWEN: bool = false, RSTDISBL: bool = false) =
  var x: uint8
...
proc write*(reg: PORTE_DDRE_Type, PE0: bool = false, PE1: bool = false, PE2: bool = false, PE3: bool = false) =
  var x: uint8
...
proc write*(reg: PORTE_PINE_Type, PE0: bool = false, PE1: bool = false, PE2: bool = false, PE3: bool = false) =
  var x: uint8
  x.setMask((PE0.uint8 shl 0).masked(0 .. 0))
  x.setMask((PE1.uint8 shl 1).masked(1 .. 1))
  x.setMask((PE2.uint8 shl 2).masked(2 .. 2))
...
proc write*(reg: PORTE_PORTE_Type, PE0: bool = false, PE1: bool = false, PE2: bool = false, PE3: bool = false) =
  var x: uint8
  x.setMask((PE0.uint8 shl 0).masked(0 .. 0))
  x.setMask((PE1.uint8 shl 1).masked(1 .. 1))
  x.setMask((PE2.uint8 shl 2).masked(2 .. 2))
...
proc write*(reg: SPI0_SPCR0_Type, SPR: SPI0_SPCR0_SPR = FOSC_2_OR_FOSC_4, CPHA: bool = false, CPOL: bool = false, MSTR: bool = false, DORD: bool = false, SPE: bool = false, SPIE: bool = false) =
  var x: uint8
...
type SPI1_SPCR1_SPR1* {.size: 1.} = enum
  FOSC_2_OR_FOSC_4 = 0x0,
  FOSC_8_OR_FOSC_16 = 0x1,
  FOSC_32_OR_FOSC_64 = 0x2,
  FOSC_64_OR_FOSC_128 = 0x3,
...
proc write*(reg: SPI1_SPCR1_Type, SPR1: SPI1_SPCR1_SPR1 = FOSC_2_OR_FOSC_4, CPHA1: bool = false, CPOL1: bool = false, MSTR1: bool = false, DORD1: bool = false, SPE1: bool = false, SPIE1: bool = false) =
  var x: uint8
  x.setMask((SPR1.uint8 shl 0).masked(0 .. 1))
  x.setMask((CPHA1.uint8 shl 2).masked(2 .. 2))
  x.setMask((CPOL1.uint8 shl 3).masked(3 .. 3))
  x.setMask((MSTR1.uint8 shl 4).masked(4 .. 4))
  x.setMask((DORD1.uint8 shl 5).masked(5 .. 5))
  x.setMask((SPE1.uint8 shl 6).masked(6 .. 6))
  x.setMask((SPIE1.uint8 shl 7).masked(7 .. 7))
...
proc write*(reg: SPI1_SPSR1_Type, SPI2X1: bool = false, WCOL1: bool = false, SPIF1: bool = false) =
  var x: uint8
  x.setMask((SPI2X1.uint8 shl 0).masked(0 .. 0))
  x.setMask((WCOL1.uint8 shl 6).masked(6 .. 6))
  x.setMask((SPIF1.uint8 shl 7).masked(7 .. 7))
...
func SPR1*(r: SPI1_SPCR1_Fields): UncheckedEnum[SPI1_SPCR1_SPR1] {.inline.} =
  toUncheckedEnum[SPI1_SPCR1_SPR1](r.uint8.bitsliced(0 .. 1).int)

proc `SPR1=`*(r: var SPI1_SPCR1_Fields, val: SPI1_SPCR1_SPR1) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 1)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 1))
  r = tmp.SPI1_SPCR1_Fields

func CPHA1*(r: SPI1_SPCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `CPHA1=`*(r: var SPI1_SPCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.SPI1_SPCR1_Fields

func CPOL1*(r: SPI1_SPCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `CPOL1=`*(r: var SPI1_SPCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.SPI1_SPCR1_Fields

func MSTR1*(r: SPI1_SPCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `MSTR1=`*(r: var SPI1_SPCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.SPI1_SPCR1_Fields

func DORD1*(r: SPI1_SPCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `DORD1=`*(r: var SPI1_SPCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.SPI1_SPCR1_Fields

func SPE1*(r: SPI1_SPCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `SPE1=`*(r: var SPI1_SPCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.SPI1_SPCR1_Fields

func SPIE1*(r: SPI1_SPCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SPIE1=`*(r: var SPI1_SPCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.SPI1_SPCR1_Fields

func SPI2X1*(r: SPI1_SPSR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `SPI2X1=`*(r: var SPI1_SPSR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.SPI1_SPSR1_Fields

func WCOL1*(r: SPI1_SPSR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `WCOL1=`*(r: var SPI1_SPSR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.SPI1_SPSR1_Fields

func SPIF1*(r: SPI1_SPSR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `SPIF1=`*(r: var SPI1_SPSR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.SPI1_SPSR1_Fields
...
proc write*(reg: TWI0_TWAMR0_Type, TWAM: uint8 = 0) =
  var x: uint8
...
proc write*(reg: TWI0_TWSR0_Type, TWPS: TWI0_TWSR0_TWPS = x1, TWS: uint8 = 0) =
  var x: uint8
...
func TWAM*(r: TWI0_TWAMR0_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(1 .. 7)

proc `TWAM=`*(r: var TWI0_TWAMR0_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 7)
...
func TWS*(r: TWI0_TWSR0_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(3 .. 7)

proc `TWS=`*(r: var TWI0_TWSR0_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 7)
...
type TWI1_TWSR1_TWPS1* {.size: 1.} = enum
  x1 = 0x0,
  x4 = 0x1,
  x16 = 0x2,
  x64 = 0x3,
...
proc read*(reg: TWI1_TWAR1_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TWI1_TWAR1_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc write*(reg: TWI1_TWCR1_Type, TWIE1: bool = false, TWEN1: bool = false, TWWC1: bool = false, TWSTO1: bool = false, TWSTA1: bool = false, TWEA1: bool = false, TWINT1: bool = false) =
  var x: uint8
  x.setMask((TWIE1.uint8 shl 0).masked(0 .. 0))
  x.setMask((TWEN1.uint8 shl 2).masked(2 .. 2))
  x.setMask((TWWC1.uint8 shl 3).masked(3 .. 3))
  x.setMask((TWSTO1.uint8 shl 4).masked(4 .. 4))
  x.setMask((TWSTA1.uint8 shl 5).masked(5 .. 5))
  x.setMask((TWEA1.uint8 shl 6).masked(6 .. 6))
  x.setMask((TWINT1.uint8 shl 7).masked(7 .. 7))
...
proc write*(reg: TWI1_TWSR1_Type, TWPS1: TWI1_TWSR1_TWPS1 = x1, TWS1: uint8 = 0) =
  var x: uint8
  x.setMask((TWPS1.uint8 shl 0).masked(0 .. 1))
  x.setMask((TWS1 shl 3).masked(3 .. 7))
...
func TWIE1*(r: TWI1_TWCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `TWIE1=`*(r: var TWI1_TWCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.TWI1_TWCR1_Fields

func TWEN1*(r: TWI1_TWCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `TWEN1=`*(r: var TWI1_TWCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.TWI1_TWCR1_Fields

func TWWC1*(r: TWI1_TWCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `TWWC1=`*(r: var TWI1_TWCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.TWI1_TWCR1_Fields

func TWSTO1*(r: TWI1_TWCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `TWSTO1=`*(r: var TWI1_TWCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.TWI1_TWCR1_Fields

func TWSTA1*(r: TWI1_TWCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `TWSTA1=`*(r: var TWI1_TWCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.TWI1_TWCR1_Fields

func TWEA1*(r: TWI1_TWCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `TWEA1=`*(r: var TWI1_TWCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.TWI1_TWCR1_Fields

func TWINT1*(r: TWI1_TWCR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `TWINT1=`*(r: var TWI1_TWCR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.TWI1_TWCR1_Fields

func TWPS1*(r: TWI1_TWSR1_Fields): UncheckedEnum[TWI1_TWSR1_TWPS1] {.inline.} =
  toUncheckedEnum[TWI1_TWSR1_TWPS1](r.uint8.bitsliced(0 .. 1).int)

proc `TWPS1=`*(r: var TWI1_TWSR1_Fields, val: TWI1_TWSR1_TWPS1) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 1)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 1))
  r = tmp.TWI1_TWSR1_Fields

func TWS1*(r: TWI1_TWSR1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(3 .. 7)

proc `TWS1=`*(r: var TWI1_TWSR1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 7)
  tmp.setMask((val shl 3).masked(3 .. 7))
  r = tmp.TWI1_TWSR1_Fields
...
proc write*(reg: USART1_UCSR1D_Type, SFDE1: bool = false, RXS1: bool = false, RXSIE1: bool = false) =
  var x: uint8
  x.setMask((SFDE1.uint8 shl 5).masked(5 .. 5))
  x.setMask((RXS1.uint8 shl 6).masked(6 .. 6))
  x.setMask((RXSIE1.uint8 shl 7).masked(7 .. 7))
...
func SFDE1*(r: USART1_UCSR1D_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `SFDE1=`*(r: var USART1_UCSR1D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.USART1_UCSR1D_Fields

func RXS1*(r: USART1_UCSR1D_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `RXS1=`*(r: var USART1_UCSR1D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.USART1_UCSR1D_Fields

func RXSIE1*(r: USART1_UCSR1D_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `RXSIE1=`*(r: var USART1_UCSR1D_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.USART1_UCSR1D_Fields
