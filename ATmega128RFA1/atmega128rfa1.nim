# Peripheral access API for ATMEGA128RFA1 microcontrollers (generated using svd2nim)
...
proc write*(reg: CPU_CLKPR_Type, CLKPS: CPU_CLKPR_CLKPS = DIVISION_FACTOR_1_RC_OSCILLATOR_2, Res: uint8 = 0, CLKPCE: bool = false) =
  var x: uint8
  x.setMask((CLKPS.uint8 shl 0).masked(0 .. 3))
  x.setMask((Res shl 4).masked(4 .. 6))
...
proc write*(reg: CPU_MCUSR_Type, PORF: bool = false, EXTRF: bool = false, BORF: bool = false, WDRF: bool = false, JTRF: bool = false, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: CPU_PRR1_Type, PRUSART1: bool = false, PRTIM3: bool = false, PRTIM4: bool = false, PRTIM5: bool = false, PRTRX24: bool = false) =
  var x: uint8
...
proc write*(reg: CPU_PRR2_Type, PRRAM0: bool = false, PRRAM1: bool = false, PRRAM2: bool = false, PRRAM3: bool = false) =
  var x: uint8
...
proc write*(reg: CPU_SMCR_Type, SE: bool = false, SM: CPU_SMCR_SM = smIDLE, Res: uint8 = 0) =
  var x: uint8
...
func Res*(r: CPU_CLKPR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 6)

proc `Res=`*(r: var CPU_CLKPR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 6)
...
func Res*(r: CPU_MCUSR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(5 .. 7)

proc `Res=`*(r: var CPU_MCUSR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 7)
...
func Res*(r: CPU_SMCR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 7)

proc `Res=`*(r: var CPU_SMCR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 7)
...
proc write*(reg: EEPROM_EECR_Type, EERE: bool = false, EEPE: bool = false, EEMPE: bool = false, EERIE: bool = false, EEPM: EEPROM_EECR_EEPM = ERASE_AND_WRITE_IN_ONE_OPERATION_ATOMIC_OPERATION, Res: uint8 = 0) =
  var x: uint8
...
func Res*(r: EEPROM_EECR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(6 .. 7)

proc `Res=`*(r: var EEPROM_EECR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
...
proc write*(reg: PWRCTRL_DPDS1_Type, PGDRV: PWRCTRL_DPDS1_PGDRV = PAD_IO_2MA, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: PWRCTRL_DRTRAM0_Type, ENDRT: bool = false, DRTSWOK: bool = false, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: PWRCTRL_DRTRAM1_Type, ENDRT: bool = false, DRTSWOK: bool = false, Res: uint8 = 0) =
  var x: uint8
  x.setMask((ENDRT.uint8 shl 4).masked(4 .. 4))
  x.setMask((DRTSWOK.uint8 shl 5).masked(5 .. 5))
  x.setMask((Res shl 6).masked(6 .. 7))
...
proc write*(reg: PWRCTRL_DRTRAM3_Type, ENDRT: bool = false, DRTSWOK: bool = false, Res: uint8 = 0) =
  var x: uint8
  x.setMask((ENDRT.uint8 shl 4).masked(4 .. 4))
  x.setMask((DRTSWOK.uint8 shl 5).masked(5 .. 5))
  x.setMask((Res shl 6).masked(6 .. 7))
...
proc write*(reg: PWRCTRL_LLDRH_Type, LLDRH: PWRCTRL_LLDRH_LLDRH = CALIBRATION_LIMIT_FOR_FAST_PROCESS_CORNER_HIGH_OUTPUT_VOLTAGE, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: PWRCTRL_LLDRL_Type, LLDRL: PWRCTRL_LLDRL_LLDRL = CALIBRATION_LIMIT_FOR_FAST_PROCESS_CORNER_HIGH_OUTPUT_VOLTAGE, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: PWRCTRL_TRXPR_Type, TRXRST: bool = false, SLPTR: bool = false, Res: uint8 = 0) =
  var x: uint8
...
func Res*(r: PWRCTRL_DPDS1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(2 .. 7)

proc `Res=`*(r: var PWRCTRL_DPDS1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 7)
  tmp.setMask((val shl 2).masked(2 .. 7))
  r = tmp.PWRCTRL_DPDS1_Fields
...
func Res*(r: PWRCTRL_DRTRAM0_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(6 .. 7)

proc `Res=`*(r: var PWRCTRL_DRTRAM0_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
  tmp.setMask((val shl 6).masked(6 .. 7))
  r = tmp.PWRCTRL_DRTRAM0_Fields
...
func Res*(r: PWRCTRL_DRTRAM1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(6 .. 7)

proc `Res=`*(r: var PWRCTRL_DRTRAM1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
  tmp.setMask((val shl 6).masked(6 .. 7))
  r = tmp.PWRCTRL_DRTRAM1_Fields
...
func Res*(r: PWRCTRL_DRTRAM3_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(6 .. 7)

proc `Res=`*(r: var PWRCTRL_DRTRAM3_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
  tmp.setMask((val shl 6).masked(6 .. 7))
  r = tmp.PWRCTRL_DRTRAM3_Fields
...
func Res*(r: PWRCTRL_LLDRH_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(5 .. 7)

proc `Res=`*(r: var PWRCTRL_LLDRH_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 7)
  tmp.setMask((val shl 5).masked(5 .. 7))
  r = tmp.PWRCTRL_LLDRH_Fields
...
func Res*(r: PWRCTRL_LLDRL_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 7)

proc `Res=`*(r: var PWRCTRL_LLDRL_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 7)
  tmp.setMask((val shl 4).masked(4 .. 7))
  r = tmp.PWRCTRL_LLDRL_Fields
...
func Res*(r: PWRCTRL_TRXPR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 7)

proc `Res=`*(r: var PWRCTRL_TRXPR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 7)
  tmp.setMask((val shl 4).masked(4 .. 7))
  r = tmp.PWRCTRL_TRXPR_Fields
...
proc write*(reg: SPI_SPCR_Type, SPR: SPI_SPCR_SPR = FOSC_4, CPHA: SPI_SPCR_CPHA = SAMPLE_LEADING_EDGE_SETUP_TRAILING_EDGE, CPOL: SPI_SPCR_CPOL = RISING_LEADING_EDGE_FALLING_TRAILING_EDGE, MSTR: bool = false, DORD: bool = false, SPE: bool = false, SPIE: bool = false) =
  var x: uint8
...
proc write*(reg: SPI_SPSR_Type, SPI2X: bool = false, Res: uint8 = 0, WCOL: bool = false, SPIF: bool = false) =
  var x: uint8
  x.setMask((SPI2X.uint8 shl 0).masked(0 .. 0))
  x.setMask((Res shl 1).masked(1 .. 5))
...
func Res*(r: SPI_SPSR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(1 .. 5)

proc `Res=`*(r: var SPI_SPSR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 5)
  tmp.setMask((val shl 1).masked(1 .. 5))
  r = tmp.SPI_SPSR_Fields
...
proc write*(reg: SYMCNT_SCCR1_Type, SCENBO: bool = false, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: TC0_GTCCR_Type, PSRSYNC: bool = false, PSRASY: bool = false, Res: uint8 = 0, TSM: bool = false) =
  var x: uint8
  x.setMask((PSRSYNC.uint8 shl 0).masked(0 .. 0))
  x.setMask((PSRASY.uint8 shl 1).masked(1 .. 1))
  x.setMask((Res shl 2).masked(2 .. 6))
...
proc write*(reg: TC0_TIFR0_Type, TOV0: bool = false, OCF0A: bool = false, OCF0B: bool = false, Res: uint8 = 0) =
  var x: uint8
...
proc write*(reg: TC0_TIMSK0_Type, TOIE0: bool = false, OCIE0A: bool = false, OCIE0B: bool = false, Res: uint8 = 0) =
  var x: uint8
...
func Res*(r: TC0_GTCCR_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(2 .. 6)

proc `Res=`*(r: var TC0_GTCCR_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 6)
...
func Res*(r: TC0_TIFR0_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(3 .. 7)

proc `Res=`*(r: var TC0_TIFR0_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 7)
  tmp.setMask((val shl 3).masked(3 .. 7))
  r = tmp.TC0_TIFR0_Fields
...
func Res*(r: TC0_TIMSK0_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(3 .. 7)

proc `Res=`*(r: var TC0_TIMSK0_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 7)
  tmp.setMask((val shl 3).masked(3 .. 7))
  r = tmp.TC0_TIMSK0_Fields
...
proc write*(reg: TC1_TCCR1C_Type, Res: uint8 = 0, FOC1C: bool = false, FOC1B: bool = false, FOC1A: bool = false) =
  var x: uint8
  x.setMask((Res shl 0).masked(0 .. 4))
...
func Res*(r: TC1_TCCR1C_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 4)

proc `Res=`*(r: var TC1_TCCR1C_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 4)
  tmp.setMask((val shl 0).masked(0 .. 4))
...
proc write*(reg: TC2_TCCR2A_Type, WGM2: TC2_TCCR2A_WGM2 = NORMAL_MODE_OF_OPERATION, Res: uint8 = 0, COM2B: TC2_TCCR2A_COM2B = NORMAL_PORT_OPERATION_OC2B_DISCONNECTED, COM2A: TC2_TCCR2A_COM2A = NORMAL_PORT_OPERATION_OC2A_DISCONNECTED) =
  var x: uint8
...
proc write*(reg: TC2_TCCR2B_Type, CS2: TC2_TCCR2B_CS2 = NO_CLOCK_SOURCE_TIMER_COUNTER2_STOPPED, WGM22: bool = false, Res: uint8 = 0, FOC2B: bool = false, FOC2A: bool = false) =
  var x: uint8
...
func Res*(r: TC2_TCCR2A_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(2 .. 3)

proc `Res=`*(r: var TC2_TCCR2A_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 3)
  tmp.setMask((val shl 2).masked(2 .. 3))
  r = tmp.TC2_TCCR2A_Fields
...
func Res*(r: TC2_TCCR2B_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 5)

proc `Res=`*(r: var TC2_TCCR2B_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 5)
  tmp.setMask((val shl 4).masked(4 .. 5))
  r = tmp.TC2_TCCR2B_Fields
...
proc write*(reg: TC3_TCCR3C_Type, Res: uint8 = 0, FOC3C: bool = false, FOC3B: bool = false, FOC3A: bool = false) =
  var x: uint8
  x.setMask((Res shl 0).masked(0 .. 4))
...
func Res*(r: TC3_TCCR3C_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 4)

proc `Res=`*(r: var TC3_TCCR3C_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 4)
  tmp.setMask((val shl 0).masked(0 .. 4))
...
proc write*(reg: TC4_TCCR4C_Type, Res: uint8 = 0, FOC4C: bool = false, FOC4B: bool = false, FOC4A: bool = false) =
  var x: uint8
  x.setMask((Res shl 0).masked(0 .. 4))
...
func Res*(r: TC4_TCCR4C_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 4)

proc `Res=`*(r: var TC4_TCCR4C_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 4)
  tmp.setMask((val shl 0).masked(0 .. 4))
...
proc write*(reg: TC5_TCCR5C_Type, Res: uint8 = 0, FOC5C: bool = false, FOC5B: bool = false, FOC5A: bool = false) =
  var x: uint8
  x.setMask((Res shl 0).masked(0 .. 4))
...
func Res*(r: TC5_TCCR5C_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 4)

proc `Res=`*(r: var TC5_TCCR5C_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 4)
  tmp.setMask((val shl 0).masked(0 .. 4))
...
type TRX24_BATMON_BATMON_VTH* {.size: 1.} = enum
  x2_550V_BATMON_HR_1_1_70V_BATMON_HR_0 = 0x0,
  x2_625V_BATMON_HR_1_1_75V_BATMON_HR_0 = 0x1,
  x2_700V_BATMON_HR_1_1_80V_BATMON_HR_0 = 0x2,
  x2_775V_BATMON_HR_1_1_85V_BATMON_HR_0 = 0x3,
  x2_850V_BATMON_HR_1_1_90V_BATMON_HR_0 = 0x4,
  x2_925V_BATMON_HR_1_1_95V_BATMON_HR_0 = 0x5,
  x3_000V_BATMON_HR_1_2_00V_BATMON_HR_0 = 0x6,
  x3_075V_BATMON_HR_1_2_05V_BATMON_HR_0 = 0x7,
  x3_150V_BATMON_HR_1_2_10V_BATMON_HR_0 = 0x8,
  x3_225V_BATMON_HR_1_2_15V_BATMON_HR_0 = 0x9,
  x3_300V_BATMON_HR_1_2_20V_BATMON_HR_0 = 0xa,
  x3_375V_BATMON_HR_1_2_25V_BATMON_HR_0 = 0xb,
  x3_450V_BATMON_HR_1_2_30V_BATMON_HR_0 = 0xc,
  x3_525V_BATMON_HR_1_2_35V_BATMON_HR_0 = 0xd,
  x3_600V_BATMON_HR_1_2_40V_BATMON_HR_0 = 0xe,
  x3_675V_BATMON_HR_1_2_45V_BATMON_HR_0 = 0xf,
...
type TRX24_PHY_TX_PWR_PA_LT* {.size: 1.} = enum
  PA_LT_2US = 0x0,
  PA_LT_4US = 0x1,
  PA_LT_6US = 0x2,
  PA_LT_8US = 0x3,

type TRX24_PHY_TX_PWR_PA_BUF_LT* {.size: 1.} = enum
  PA_BUF_LT_0US = 0x0,
  PA_BUF_LT_2US = 0x1,
  PA_BUF_LT_4US = 0x2,
  PA_BUF_LT_6US = 0x3,
...
proc write*(reg: TRX24_AES_STATUS_Type, AES_DONE: bool = false, Res: uint8 = 0, AES_ER: bool = false) =
  var x: uint8
  x.setMask((AES_DONE.uint8 shl 0).masked(0 .. 0))
  x.setMask((Res shl 1).masked(1 .. 6))
...
proc write*(reg: TRX24_ANT_DIV_Type, ANT_CTRL: TRX24_ANT_DIV_ANT_CTRL = RESERVED, ANT_EXT_SW_EN: TRX24_ANT_DIV_ANT_EXT_SW_EN = ANT_DIV_EXT_SW_DIS, ANT_DIV_EN: TRX24_ANT_DIV_ANT_DIV_EN = ANTENNA_DIVERSITY_ALGORITHM_DISABLED, Res: uint8 = 0, ANT_SEL: TRX24_ANT_DIV_ANT_SEL = ANTENNA_0) =
  var x: uint8
...
proc write*(reg: TRX24_BATMON_Type, BATMON_VTH: TRX24_BATMON_BATMON_VTH = x2_550V_BATMON_HR_1_1_70V_BATMON_HR_0, BATMON_HR: TRX24_BATMON_BATMON_HR = BATMON_HR_DIS, BATMON_OK: TRX24_BATMON_BATMON_OK = THE_BATTERY_VOLTAGE_IS_BELOW_THE_THRESHOLD, BAT_LOW_EN: bool = false, BAT_LOW: bool = false) =
  var x: uint8
...
proc write*(reg: TRX24_PHY_TX_PWR_Type, TX_PWR: TRX24_PHY_TX_PWR_TX_PWR = x3_0_DBM, PA_LT: TRX24_PHY_TX_PWR_PA_LT = PA_LT_2US, PA_BUF_LT: TRX24_PHY_TX_PWR_PA_BUF_LT = PA_BUF_LT_0US) =
  var x: uint8
  x.setMask((TX_PWR.uint8 shl 0).masked(0 .. 3))
  x.setMask((PA_LT.uint8 shl 4).masked(4 .. 5))
  x.setMask((PA_BUF_LT.uint8 shl 6).masked(6 .. 7))
...
proc write*(reg: TRX24_RX_SYN_Type, RX_PDT_LEVEL: TRX24_RX_SYN_RX_PDT_LEVEL = RX_PDT_LEVEL_MIN, Res: uint8 = 0, RX_PDT_DIS: bool = false) =
  var x: uint8
...
proc read*(reg: TRX24_TRX_CTRL_0_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: TRX24_TRX_CTRL_0_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)
...
proc write*(reg: TRX24_TRX_CTRL_1_Type, Res: uint8 = 0, TX_AUTO_CRC_ON: bool = false, IRQ_2_EXT_EN: bool = false, PA_EXT_EN: bool = false) =
  var x: uint8
  x.setMask((Res shl 0).masked(0 .. 4))
...
proc write*(reg: TRX24_TRX_CTRL_2_Type, OQPSK_DATA_RATE: TRX24_TRX_CTRL_2_OQPSK_DATA_RATE = RATE_250KB, Res: uint8 = 0, RX_SAFE_MODE: bool = false) =
  var x: uint8
...
func Res*(r: TRX24_AES_STATUS_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(1 .. 6)

proc `Res=`*(r: var TRX24_AES_STATUS_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 6)
...
func Res*(r: TRX24_ANT_DIV_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 6)

proc `Res=`*(r: var TRX24_ANT_DIV_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 6)
  tmp.setMask((val shl 4).masked(4 .. 6))
  r = tmp.TRX24_ANT_DIV_Fields
...
func PA_LT*(r: TRX24_PHY_TX_PWR_Fields): UncheckedEnum[TRX24_PHY_TX_PWR_PA_LT] {.inline.} =
  toUncheckedEnum[TRX24_PHY_TX_PWR_PA_LT](r.uint8.bitsliced(4 .. 5).int)

proc `PA_LT=`*(r: var TRX24_PHY_TX_PWR_Fields, val: TRX24_PHY_TX_PWR_PA_LT) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 5)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 5))
  r = tmp.TRX24_PHY_TX_PWR_Fields

func PA_BUF_LT*(r: TRX24_PHY_TX_PWR_Fields): UncheckedEnum[TRX24_PHY_TX_PWR_PA_BUF_LT] {.inline.} =
  toUncheckedEnum[TRX24_PHY_TX_PWR_PA_BUF_LT](r.uint8.bitsliced(6 .. 7).int)

proc `PA_BUF_LT=`*(r: var TRX24_PHY_TX_PWR_Fields, val: TRX24_PHY_TX_PWR_PA_BUF_LT) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 7))
  r = tmp.TRX24_PHY_TX_PWR_Fields
...
func Res*(r: TRX24_RX_SYN_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 6)

proc `Res=`*(r: var TRX24_RX_SYN_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 6)
  tmp.setMask((val shl 4).masked(4 .. 6))
  r = tmp.TRX24_RX_SYN_Fields
...
func Res*(r: TRX24_TRX_CTRL_1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 4)

proc `Res=`*(r: var TRX24_TRX_CTRL_1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 4)
  tmp.setMask((val shl 0).masked(0 .. 4))
...
func Res*(r: TRX24_TRX_CTRL_2_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(2 .. 6)

proc `Res=`*(r: var TRX24_TRX_CTRL_2_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 6)
  tmp.setMask((val shl 2).masked(2 .. 6))
  r = tmp.TRX24_TRX_CTRL_2_Fields
