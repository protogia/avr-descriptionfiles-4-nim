# Peripheral access API for AT90PWM3B microcontrollers (generated using svd2nim)
...
type EUSART_EUCSRA_Type* = object
  loc: uint

type EUSART_EUCSRB_Type* = object
  loc: uint

type EUSART_EUCSRC_Type* = object
  loc: uint

type EUSART_EUDR_Type* = object
  loc: uint

type EUSART_MUBRR_Type* = object
  loc: uint

type EUSART_Type* = object
  EUCSRA*: EUSART_EUCSRA_Type
  EUCSRB*: EUSART_EUCSRB_Type
  EUCSRC*: EUSART_EUCSRC_Type
  MUBRR*: EUSART_MUBRR_Type
  EUDR*: EUSART_EUDR_Type
...
type PSC1_OCR1RA_Type* = object
  loc: uint

type PSC1_OCR1RB_Type* = object
  loc: uint

type PSC1_OCR1SA_Type* = object
  loc: uint

type PSC1_OCR1SB_Type* = object
  loc: uint

type PSC1_PCNF1_Type* = object
  loc: uint
...
type PSC1_PIFR1_Type* = object
  loc: uint

type PSC1_PIM1_Type* = object
  loc: uint
...
type PSC1_Type* = object
  PIFR1*: PSC1_PIFR1_Type
  PIM1*: PSC1_PIM1_Type
  PSOC1*: PSC1_PSOC1_Type
  OCR1SA*: PSC1_OCR1SA_Type
  OCR1RA*: PSC1_OCR1RA_Type
  OCR1SB*: PSC1_OCR1SB_Type
  OCR1RB*: PSC1_OCR1RB_Type
  PCNF1*: PSC1_PCNF1_Type
...
const AC* = AC_Type(
  ...
  AC1CON: AC_AC1CON_Type(loc: 0xae'u),
...
const DAC* = DAC_Type(
  DACON: DAC_DACON_Type(loc: 0xaa'u),
  DAC: DAC_DAC_Type(loc: 0xab'u),
)
...
const EUSART* = EUSART_Type(
  EUCSRA: EUSART_EUCSRA_Type(loc: 0xc8'u),
  EUCSRB: EUSART_EUCSRB_Type(loc: 0xc9'u),
  EUCSRC: EUSART_EUCSRC_Type(loc: 0xca'u),
  MUBRR: EUSART_MUBRR_Type(loc: 0xcc'u),
  EUDR: EUSART_EUDR_Type(loc: 0xce'u),
)
...
const PSC1* = PSC1_Type(
  PIFR1: PSC1_PIFR1_Type(loc: 0xa2'u),
  PIM1: PSC1_PIM1_Type(loc: 0xa3'u),
  PSOC1: PSC1_PSOC1_Type(loc: 0xe0'u),
  OCR1SA: PSC1_OCR1SA_Type(loc: 0xe2'u),
  OCR1RA: PSC1_OCR1RA_Type(loc: 0xe4'u),
  OCR1SB: PSC1_OCR1SB_Type(loc: 0xe6'u),
  OCR1RB: PSC1_OCR1RB_Type(loc: 0xe8'u),
  PCNF1: PSC1_PCNF1_Type(loc: 0xea'u),
...
const USART* = USART_Type(
  UCSRA: USART_UCSRA_Type(loc: 0xc0'u),
  UCSRB: USART_UCSRB_Type(loc: 0xc1'u),
  UCSRC: USART_UCSRC_Type(loc: 0xc2'u),
  UBRR: USART_UBRR_Type(loc: 0xc4'u),
  UDR: USART_UDR_Type(loc: 0xc6'u),
)
...
proc write*(reg: ADC_ADCSRB_Type, ADTS: uint8 = 0, ADHSM: bool = false) =
  var x: uint8
  x.setMask((ADTS shl 0).masked(0 .. 3))
...
type
  EUSART_EUCSRA_Fields* = distinct uint8
  EUSART_EUCSRB_Fields* = distinct uint8
  EUSART_EUCSRC_Fields* = distinct uint8

type EUSART_EUCSRA_URxS* {.size: 1.} = enum
  x5_BIT = 0x0,
  x6_BIT = 0x1,
  x7_BIT = 0x2,
  x8_BIT = 0x3,
...
type EUSART_EUCSRA_UTxS* {.size: 1.} = enum
  x5_BIT = 0x0,
  x6_BIT = 0x1,
  x7_BIT = 0x2,
  x8_BIT = 0x3,
...
proc read*(reg: EUSART_EUCSRA_Type): EUSART_EUCSRA_Fields {.inline.} =
  volatileLoad(cast[ptr EUSART_EUCSRA_Fields](reg.loc))

proc write*(reg: EUSART_EUCSRA_Type, val: EUSART_EUCSRA_Fields) {.inline.} =
  volatileStore(cast[ptr EUSART_EUCSRA_Fields](reg.loc), val)

proc write*(reg: EUSART_EUCSRA_Type, URxS: EUSART_EUCSRA_URxS = x5_BIT, UTxS: EUSART_EUCSRA_UTxS = x5_BIT) =
  var x: uint8
  x.setMask((URxS.uint8 shl 0).masked(0 .. 3))
  x.setMask((UTxS.uint8 shl 4).masked(4 .. 7))
  reg.write x.EUSART_EUCSRA_Fields

template modifyIt*(reg: EUSART_EUCSRA_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: EUSART_EUCSRB_Type): EUSART_EUCSRB_Fields {.inline.} =
  volatileLoad(cast[ptr EUSART_EUCSRB_Fields](reg.loc))

proc write*(reg: EUSART_EUCSRB_Type, val: EUSART_EUCSRB_Fields) {.inline.} =
  volatileStore(cast[ptr EUSART_EUCSRB_Fields](reg.loc), val)

proc write*(reg: EUSART_EUCSRB_Type, BODR: bool = false, EMCH: bool = false, EUSBS: bool = false, EUSART: bool = false) =
  var x: uint8
  x.setMask((BODR.uint8 shl 0).masked(0 .. 0))
  x.setMask((EMCH.uint8 shl 1).masked(1 .. 1))
  x.setMask((EUSBS.uint8 shl 3).masked(3 .. 3))
  x.setMask((EUSART.uint8 shl 4).masked(4 .. 4))
  reg.write x.EUSART_EUCSRB_Fields

template modifyIt*(reg: EUSART_EUCSRB_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: EUSART_EUCSRC_Type): EUSART_EUCSRC_Fields {.inline.} =
  volatileLoad(cast[ptr EUSART_EUCSRC_Fields](reg.loc))

proc write*(reg: EUSART_EUCSRC_Type, val: EUSART_EUCSRC_Fields) {.inline.} =
  volatileStore(cast[ptr EUSART_EUCSRC_Fields](reg.loc), val)

proc write*(reg: EUSART_EUCSRC_Type, STP: uint8 = 0, F1617: bool = false, FEM: bool = false) =
  var x: uint8
  x.setMask((STP shl 0).masked(0 .. 1))
  x.setMask((F1617.uint8 shl 2).masked(2 .. 2))
  x.setMask((FEM.uint8 shl 3).masked(3 .. 3))
  reg.write x.EUSART_EUCSRC_Fields

template modifyIt*(reg: EUSART_EUCSRC_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: EUSART_EUDR_Type): uint8 {.inline.} =
  volatileLoad(cast[ptr uint8](reg.loc))

proc write*(reg: EUSART_EUDR_Type, val: uint8) {.inline.} =
  volatileStore(cast[ptr uint8](reg.loc), val)

template modifyIt*(reg: EUSART_EUDR_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: EUSART_MUBRR_Type): uint16 {.inline.} =
  volatileLoad(cast[ptr uint16](reg.loc))

proc write*(reg: EUSART_MUBRR_Type, val: uint16) {.inline.} =
  volatileStore(cast[ptr uint16](reg.loc), val)

template modifyIt*(reg: EUSART_MUBRR_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

func URxS*(r: EUSART_EUCSRA_Fields): UncheckedEnum[EUSART_EUCSRA_URxS] {.inline.} =
  toUncheckedEnum[EUSART_EUCSRA_URxS](r.uint8.bitsliced(0 .. 3).int)

proc `URxS=`*(r: var EUSART_EUCSRA_Fields, val: EUSART_EUCSRA_URxS) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 3)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 3))
  r = tmp.EUSART_EUCSRA_Fields

func UTxS*(r: EUSART_EUCSRA_Fields): UncheckedEnum[EUSART_EUCSRA_UTxS] {.inline.} =
  toUncheckedEnum[EUSART_EUCSRA_UTxS](r.uint8.bitsliced(4 .. 7).int)

proc `UTxS=`*(r: var EUSART_EUCSRA_Fields, val: EUSART_EUCSRA_UTxS) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 7)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 7))
  r = tmp.EUSART_EUCSRA_Fields

func BODR*(r: EUSART_EUCSRB_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `BODR=`*(r: var EUSART_EUCSRB_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.EUSART_EUCSRB_Fields

func EMCH*(r: EUSART_EUCSRB_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `EMCH=`*(r: var EUSART_EUCSRB_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.EUSART_EUCSRB_Fields

func EUSBS*(r: EUSART_EUCSRB_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `EUSBS=`*(r: var EUSART_EUCSRB_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.EUSART_EUCSRB_Fields

func EUSARTField*(r: EUSART_EUCSRB_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `EUSARTField=`*(r: var EUSART_EUCSRB_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.EUSART_EUCSRB_Fields

func STP*(r: EUSART_EUCSRC_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 1)

proc `STP=`*(r: var EUSART_EUCSRC_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 1)
  tmp.setMask((val shl 0).masked(0 .. 1))
  r = tmp.EUSART_EUCSRC_Fields

func F1617*(r: EUSART_EUCSRC_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `F1617=`*(r: var EUSART_EUCSRC_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.EUSART_EUCSRC_Fields

func FEM*(r: EUSART_EUCSRC_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `FEM=`*(r: var EUSART_EUCSRC_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.EUSART_EUCSRC_Fields
...
proc write*(reg: FUSE_EXTENDED_Type, BOOTRST: bool = false, BOOTSZ: FUSE_EXTENDED_BOOTSZ = x1024W_0C00, PSCRV: bool = false, PSC0RB: bool = false, PSC1RB: bool = false, PSC2RB: bool = false) =
  var x: uint8
  ...
  x.setMask((PSC1RB.uint8 shl 6).masked(6 .. 6))
...
func PSC1RB*(r: FUSE_EXTENDED_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `PSC1RB=`*(r: var FUSE_EXTENDED_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.FUSE_EXTENDED_Fields
...
type
  PSC0_OCR0RA_Fields* = distinct uint16
  PSC0_OCR0SA_Fields* = distinct uint16
  PSC0_OCR0SB_Fields* = distinct uint16
...
proc read*(reg: PSC0_OCR0RA_Type): PSC0_OCR0RA_Fields {.inline.} =
  volatileLoad(cast[ptr PSC0_OCR0RA_Fields](reg.loc))

proc write*(reg: PSC0_OCR0RA_Type, val: PSC0_OCR0RA_Fields) {.inline.} =
  volatileStore(cast[ptr PSC0_OCR0RA_Fields](reg.loc), val)

proc write*(reg: PSC0_OCR0RA_Type, OCR0RA: uint16 = 0) =
  var x: uint16
  x.setMask((OCR0RA shl 0).masked(0 .. 11))
  reg.write x.PSC0_OCR0RA_Fields
...
proc read*(reg: PSC0_OCR0SA_Type): PSC0_OCR0SA_Fields {.inline.} =
  volatileLoad(cast[ptr PSC0_OCR0SA_Fields](reg.loc))

proc write*(reg: PSC0_OCR0SA_Type, val: PSC0_OCR0SA_Fields) {.inline.} =
  volatileStore(cast[ptr PSC0_OCR0SA_Fields](reg.loc), val)

proc write*(reg: PSC0_OCR0SA_Type, OCR0SA: uint16 = 0) =
  var x: uint16
  x.setMask((OCR0SA shl 0).masked(0 .. 11))
  reg.write x.PSC0_OCR0SA_Fields
...
proc read*(reg: PSC0_OCR0SB_Type): PSC0_OCR0SB_Fields {.inline.} =
  volatileLoad(cast[ptr PSC0_OCR0SB_Fields](reg.loc))

proc write*(reg: PSC0_OCR0SB_Type, val: PSC0_OCR0SB_Fields) {.inline.} =
  volatileStore(cast[ptr PSC0_OCR0SB_Fields](reg.loc), val)

proc write*(reg: PSC0_OCR0SB_Type, OCR0SB: uint16 = 0) =
  var x: uint16
  x.setMask((OCR0SB shl 0).masked(0 .. 11))
  reg.write x.PSC0_OCR0SB_Fields
...
proc write*(reg: PSC0_PCTL0_Type, PRUN0: bool = false, PCCYC0: bool = false, PARUN0: bool = false, PAOC0A: bool = false, PAOC0B: bool = false, PBFM0: bool = false, PPRE0: uint8 = 0) =
  var x: uint8
...
proc write*(reg: PSC0_PIFR0_Type, PEOP0: bool = false, PRN0: uint8 = 0, PEV0A: bool = false, PEV0B: bool = false, PSEI0: bool = false, POAC0A: bool = false, POAC0B: bool = false) =
  var x: uint8
...
proc write*(reg: PSC0_PSOC0_Type, POEN0A: bool = false, POEN0B: bool = false, PSYNC0: uint8 = 0) =
  var x: uint8
...
func OCR0RA*(r: PSC0_OCR0RA_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR0RA=`*(r: var PSC0_OCR0RA_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC0_OCR0RA_Fields

func OCR0SA*(r: PSC0_OCR0SA_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR0SA=`*(r: var PSC0_OCR0SA_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC0_OCR0SA_Fields

func OCR0SB*(r: PSC0_OCR0SB_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR0SB=`*(r: var PSC0_OCR0SB_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC0_OCR0SB_Fields
...
type
  PSC1_OCR1RA_Fields* = distinct uint16
  PSC1_OCR1SA_Fields* = distinct uint16
  PSC1_OCR1SB_Fields* = distinct uint16
  PSC1_PCNF1_Fields* = distinct uint8
  PSC1_PCTL1_Fields* = distinct uint8
  PSC1_PFRC1A_Fields* = distinct uint8
  PSC1_PFRC1B_Fields* = distinct uint8
  PSC1_PICR1_Fields* = distinct uint16
  PSC1_PIFR1_Fields* = distinct uint8
  PSC1_PIM1_Fields* = distinct uint8
  PSC1_PSOC1_Fields* = distinct uint8

proc read*(reg: PSC1_OCR1RA_Type): PSC1_OCR1RA_Fields {.inline.} =
  volatileLoad(cast[ptr PSC1_OCR1RA_Fields](reg.loc))

proc write*(reg: PSC1_OCR1RA_Type, val: PSC1_OCR1RA_Fields) {.inline.} =
  volatileStore(cast[ptr PSC1_OCR1RA_Fields](reg.loc), val)

proc write*(reg: PSC1_OCR1RA_Type, OCR1RA: uint16 = 0) =
  var x: uint16
  x.setMask((OCR1RA shl 0).masked(0 .. 11))
  reg.write x.PSC1_OCR1RA_Fields

template modifyIt*(reg: PSC1_OCR1RA_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: PSC1_OCR1RB_Type): uint16 {.inline.} =
  volatileLoad(cast[ptr uint16](reg.loc))

proc write*(reg: PSC1_OCR1RB_Type, val: uint16) {.inline.} =
  volatileStore(cast[ptr uint16](reg.loc), val)

template modifyIt*(reg: PSC1_OCR1RB_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: PSC1_OCR1SA_Type): PSC1_OCR1SA_Fields {.inline.} =
  volatileLoad(cast[ptr PSC1_OCR1SA_Fields](reg.loc))

proc write*(reg: PSC1_OCR1SA_Type, val: PSC1_OCR1SA_Fields) {.inline.} =
  volatileStore(cast[ptr PSC1_OCR1SA_Fields](reg.loc), val)

proc write*(reg: PSC1_OCR1SA_Type, OCR1SA: uint16 = 0) =
  var x: uint16
  x.setMask((OCR1SA shl 0).masked(0 .. 11))
  reg.write x.PSC1_OCR1SA_Fields

template modifyIt*(reg: PSC1_OCR1SA_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: PSC1_OCR1SB_Type): PSC1_OCR1SB_Fields {.inline.} =
  volatileLoad(cast[ptr PSC1_OCR1SB_Fields](reg.loc))

proc write*(reg: PSC1_OCR1SB_Type, val: PSC1_OCR1SB_Fields) {.inline.} =
  volatileStore(cast[ptr PSC1_OCR1SB_Fields](reg.loc), val)

proc write*(reg: PSC1_OCR1SB_Type, OCR1SB: uint16 = 0) =
  var x: uint16
  x.setMask((OCR1SB shl 0).masked(0 .. 11))
  reg.write x.PSC1_OCR1SB_Fields

template modifyIt*(reg: PSC1_OCR1SB_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: PSC1_PCNF1_Type): PSC1_PCNF1_Fields {.inline.} =
  volatileLoad(cast[ptr PSC1_PCNF1_Fields](reg.loc))

proc write*(reg: PSC1_PCNF1_Type, val: PSC1_PCNF1_Fields) {.inline.} =
  volatileStore(cast[ptr PSC1_PCNF1_Fields](reg.loc), val)

proc write*(reg: PSC1_PCNF1_Type, PCLKSEL1: bool = false, POP1: bool = false, PMODE1: uint8 = 0, PLOCK1: bool = false, PALOCK1: bool = false, PFIFTY1: bool = false) =
  var x: uint8
  x.setMask((PCLKSEL1.uint8 shl 1).masked(1 .. 1))
  x.setMask((POP1.uint8 shl 2).masked(2 .. 2))
  x.setMask((PMODE1 shl 3).masked(3 .. 4))
  x.setMask((PLOCK1.uint8 shl 5).masked(5 .. 5))
  x.setMask((PALOCK1.uint8 shl 6).masked(6 .. 6))
  x.setMask((PFIFTY1.uint8 shl 7).masked(7 .. 7))
  reg.write x.PSC1_PCNF1_Fields

template modifyIt*(reg: PSC1_PCNF1_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: PSC1_PCTL1_Type, PRUN1: bool = false, PCCYC1: bool = false, PARUN1: bool = false, PAOC1A: bool = false, PAOC1B: bool = false, PBFM1: bool = false, PPRE1: uint8 = 0) =
  var x: uint8
  ...
  x.setMask((PPRE1 shl 6).masked(6 .. 7))
...
proc write*(reg: PSC1_PFRC1A_Type, PRFM1A: uint8 = 0, PFLTE1A: bool = false, PELEV1A: bool = false, PISEL1A: bool = false, PCAE1A: bool = false) =
  var x: uint8
  x.setMask((PRFM1A shl 0).masked(0 .. 3))
...
proc write*(reg: PSC1_PFRC1B_Type, PRFM1B: uint8 = 0, PFLTE1B: bool = false, PELEV1B: bool = false, PISEL1B: bool = false, PCAE1B: bool = false) =
  var x: uint8
  x.setMask((PRFM1B shl 0).masked(0 .. 3))
...
proc read*(reg: PSC1_PICR1_Type): PSC1_PICR1_Fields {.inline.} =
  volatileLoad(cast[ptr PSC1_PICR1_Fields](reg.loc))

proc write*(reg: PSC1_PICR1_Type, val: PSC1_PICR1_Fields) {.inline.} =
  volatileStore(cast[ptr PSC1_PICR1_Fields](reg.loc), val)

proc write*(reg: PSC1_PICR1_Type, PICR1: uint16 = 0, PCST1: bool = false) =
  var x: uint16
  x.setMask((PICR1 shl 0).masked(0 .. 11))
  x.setMask((PCST1.uint16 shl 15).masked(15 .. 15))
  reg.write x.PSC1_PICR1_Fields
...
proc read*(reg: PSC1_PIFR1_Type): PSC1_PIFR1_Fields {.inline.} =
  volatileLoad(cast[ptr PSC1_PIFR1_Fields](reg.loc))

proc write*(reg: PSC1_PIFR1_Type, val: PSC1_PIFR1_Fields) {.inline.} =
  volatileStore(cast[ptr PSC1_PIFR1_Fields](reg.loc), val)

proc write*(reg: PSC1_PIFR1_Type, PEOP1: bool = false, PRN1: uint8 = 0, PEV1A: bool = false, PEV1B: bool = false, PSEI1: bool = false, POAC1A: bool = false, POAC1B: bool = false) =
  var x: uint8
  x.setMask((PEOP1.uint8 shl 0).masked(0 .. 0))
  x.setMask((PRN1 shl 1).masked(1 .. 2))
  x.setMask((PEV1A.uint8 shl 3).masked(3 .. 3))
  x.setMask((PEV1B.uint8 shl 4).masked(4 .. 4))
  x.setMask((PSEI1.uint8 shl 5).masked(5 .. 5))
  x.setMask((POAC1A.uint8 shl 6).masked(6 .. 6))
  x.setMask((POAC1B.uint8 shl 7).masked(7 .. 7))
  reg.write x.PSC1_PIFR1_Fields

template modifyIt*(reg: PSC1_PIFR1_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: PSC1_PIM1_Type): PSC1_PIM1_Fields {.inline.} =
  volatileLoad(cast[ptr PSC1_PIM1_Fields](reg.loc))

proc write*(reg: PSC1_PIM1_Type, val: PSC1_PIM1_Fields) {.inline.} =
  volatileStore(cast[ptr PSC1_PIM1_Fields](reg.loc), val)

proc write*(reg: PSC1_PIM1_Type, PEOPE1: bool = false, PEVE1A: bool = false, PEVE1B: bool = false, PSEIE1: bool = false) =
  var x: uint8
  x.setMask((PEOPE1.uint8 shl 0).masked(0 .. 0))
  x.setMask((PEVE1A.uint8 shl 3).masked(3 .. 3))
  x.setMask((PEVE1B.uint8 shl 4).masked(4 .. 4))
  x.setMask((PSEIE1.uint8 shl 5).masked(5 .. 5))
  reg.write x.PSC1_PIM1_Fields

template modifyIt*(reg: PSC1_PIM1_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: PSC1_PSOC1_Type, POEN1A: bool = false, POEN1B: bool = false, PSYNC1: uint8 = 0) =
  var x: uint8
  x.setMask((POEN1A.uint8 shl 0).masked(0 .. 0))
  x.setMask((POEN1B.uint8 shl 2).masked(2 .. 2))
  x.setMask((PSYNC1 shl 4).masked(4 .. 5))
...
func OCR1RA*(r: PSC1_OCR1RA_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR1RA=`*(r: var PSC1_OCR1RA_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC1_OCR1RA_Fields

func OCR1SA*(r: PSC1_OCR1SA_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR1SA=`*(r: var PSC1_OCR1SA_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC1_OCR1SA_Fields

func OCR1SB*(r: PSC1_OCR1SB_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR1SB=`*(r: var PSC1_OCR1SB_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC1_OCR1SB_Fields

func PCLKSEL1*(r: PSC1_PCNF1_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `PCLKSEL1=`*(r: var PSC1_PCNF1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.PSC1_PCNF1_Fields

func POP1*(r: PSC1_PCNF1_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `POP1=`*(r: var PSC1_PCNF1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.PSC1_PCNF1_Fields

func PMODE1*(r: PSC1_PCNF1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(3 .. 4)

proc `PMODE1=`*(r: var PSC1_PCNF1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 4)
  tmp.setMask((val shl 3).masked(3 .. 4))
  r = tmp.PSC1_PCNF1_Fields

func PLOCK1*(r: PSC1_PCNF1_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `PLOCK1=`*(r: var PSC1_PCNF1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.PSC1_PCNF1_Fields

func PALOCK1*(r: PSC1_PCNF1_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `PALOCK1=`*(r: var PSC1_PCNF1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.PSC1_PCNF1_Fields

func PFIFTY1*(r: PSC1_PCNF1_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `PFIFTY1=`*(r: var PSC1_PCNF1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.PSC1_PCNF1_Fields
...
func PPRE1*(r: PSC1_PCTL1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(6 .. 7)

proc `PPRE1=`*(r: var PSC1_PCTL1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
  tmp.setMask((val shl 6).masked(6 .. 7))
  r = tmp.PSC1_PCTL1_Fields

func PRFM1A*(r: PSC1_PFRC1A_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 3)

proc `PRFM1A=`*(r: var PSC1_PFRC1A_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 3)
  tmp.setMask((val shl 0).masked(0 .. 3))
...
func PRFM1B*(r: PSC1_PFRC1B_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 3)

proc `PRFM1B=`*(r: var PSC1_PFRC1B_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 3)
  tmp.setMask((val shl 0).masked(0 .. 3))
...
func PICR1*(r: PSC1_PICR1_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `PICR1=`*(r: var PSC1_PICR1_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC1_PICR1_Fields

func PCST1*(r: PSC1_PICR1_Fields): bool {.inline.} =
  r.uint16.bitsliced(15 .. 15).bool

proc `PCST1=`*(r: var PSC1_PICR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(15 .. 15)
  tmp.setMask((val.uint16 shl 15).masked(15 .. 15))
  r = tmp.PSC1_PICR1_Fields

func PEOP1*(r: PSC1_PIFR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `PEOP1=`*(r: var PSC1_PIFR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.PSC1_PIFR1_Fields

func PRN1*(r: PSC1_PIFR1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(1 .. 2)

proc `PRN1=`*(r: var PSC1_PIFR1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 2)
  tmp.setMask((val shl 1).masked(1 .. 2))
  r = tmp.PSC1_PIFR1_Fields

func PEV1A*(r: PSC1_PIFR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `PEV1A=`*(r: var PSC1_PIFR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.PSC1_PIFR1_Fields

func PEV1B*(r: PSC1_PIFR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `PEV1B=`*(r: var PSC1_PIFR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.PSC1_PIFR1_Fields

func PSEI1*(r: PSC1_PIFR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `PSEI1=`*(r: var PSC1_PIFR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.PSC1_PIFR1_Fields

func POAC1A*(r: PSC1_PIFR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `POAC1A=`*(r: var PSC1_PIFR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.PSC1_PIFR1_Fields

func POAC1B*(r: PSC1_PIFR1_Fields): bool {.inline.} =
  r.uint8.bitsliced(7 .. 7).bool

proc `POAC1B=`*(r: var PSC1_PIFR1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(7 .. 7)
  tmp.setMask((val.uint8 shl 7).masked(7 .. 7))
  r = tmp.PSC1_PIFR1_Fields

func PEOPE1*(r: PSC1_PIM1_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `PEOPE1=`*(r: var PSC1_PIM1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.PSC1_PIM1_Fields

func PEVE1A*(r: PSC1_PIM1_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `PEVE1A=`*(r: var PSC1_PIM1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.PSC1_PIM1_Fields

func PEVE1B*(r: PSC1_PIM1_Fields): bool {.inline.} =
  r.uint8.bitsliced(4 .. 4).bool

proc `PEVE1B=`*(r: var PSC1_PIM1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 4)
  tmp.setMask((val.uint8 shl 4).masked(4 .. 4))
  r = tmp.PSC1_PIM1_Fields

func PSEIE1*(r: PSC1_PIM1_Fields): bool {.inline.} =
  r.uint8.bitsliced(5 .. 5).bool

proc `PSEIE1=`*(r: var PSC1_PIM1_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(5 .. 5)
  tmp.setMask((val.uint8 shl 5).masked(5 .. 5))
  r = tmp.PSC1_PIM1_Fields
...
func PSYNC1*(r: PSC1_PSOC1_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 5)

proc `PSYNC1=`*(r: var PSC1_PSOC1_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 5)
  tmp.setMask((val shl 4).masked(4 .. 5))
  r = tmp.PSC1_PSOC1_Fields
...
type
  PSC2_OCR2RA_Fields* = distinct uint16
  PSC2_OCR2SA_Fields* = distinct uint16
  PSC2_OCR2SB_Fields* = distinct uint16
...
proc read*(reg: PSC2_OCR2RA_Type): PSC2_OCR2RA_Fields {.inline.} =
  volatileLoad(cast[ptr PSC2_OCR2RA_Fields](reg.loc))

proc write*(reg: PSC2_OCR2RA_Type, val: PSC2_OCR2RA_Fields) {.inline.} =
  volatileStore(cast[ptr PSC2_OCR2RA_Fields](reg.loc), val)

proc write*(reg: PSC2_OCR2RA_Type, OCR2RA: uint16 = 0) =
  var x: uint16
  x.setMask((OCR2RA shl 0).masked(0 .. 11))
  reg.write x.PSC2_OCR2RA_Fields
...
proc read*(reg: PSC2_OCR2SA_Type): PSC2_OCR2SA_Fields {.inline.} =
  volatileLoad(cast[ptr PSC2_OCR2SA_Fields](reg.loc))

proc write*(reg: PSC2_OCR2SA_Type, val: PSC2_OCR2SA_Fields) {.inline.} =
  volatileStore(cast[ptr PSC2_OCR2SA_Fields](reg.loc), val)

proc write*(reg: PSC2_OCR2SA_Type, OCR2SA: uint16 = 0) =
  var x: uint16
  x.setMask((OCR2SA shl 0).masked(0 .. 11))
  reg.write x.PSC2_OCR2SA_Fields
...
proc read*(reg: PSC2_OCR2SB_Type): PSC2_OCR2SB_Fields {.inline.} =
  volatileLoad(cast[ptr PSC2_OCR2SB_Fields](reg.loc))

proc write*(reg: PSC2_OCR2SB_Type, val: PSC2_OCR2SB_Fields) {.inline.} =
  volatileStore(cast[ptr PSC2_OCR2SB_Fields](reg.loc), val)

proc write*(reg: PSC2_OCR2SB_Type, OCR2SB: uint16 = 0) =
  var x: uint16
  x.setMask((OCR2SB shl 0).masked(0 .. 11))
  reg.write x.PSC2_OCR2SB_Fields
...
func OCR2RA*(r: PSC2_OCR2RA_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR2RA=`*(r: var PSC2_OCR2RA_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC2_OCR2RA_Fields

func OCR2SA*(r: PSC2_OCR2SA_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR2SA=`*(r: var PSC2_OCR2SA_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC2_OCR2SA_Fields

func OCR2SB*(r: PSC2_OCR2SB_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `OCR2SB=`*(r: var PSC2_OCR2SB_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.PSC2_OCR2SB_Fields
...
proc write*(reg: TC0_GTCCR_Type, PSR10: bool = false, ICPSEL1: bool = false, TSM: bool = false) =
  var x: uint8
...
type
  USART_UBRR_Fields* = distinct uint16
...
proc read*(reg: USART_UBRR_Type): USART_UBRR_Fields {.inline.} =
  volatileLoad(cast[ptr USART_UBRR_Fields](reg.loc))

proc write*(reg: USART_UBRR_Type, val: USART_UBRR_Fields) {.inline.} =
  volatileStore(cast[ptr USART_UBRR_Fields](reg.loc), val)

proc write*(reg: USART_UBRR_Type, UBRR: uint16 = 0) =
  var x: uint16
  x.setMask((UBRR shl 0).masked(0 .. 11))
  reg.write x.USART_UBRR_Fields

template modifyIt*(reg: USART_UBRR_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)
...
proc write*(reg: USART_UCSRC_Type, UCPOL: bool = false, UCSZ: uint8 = 0, USBS: USART_UCSRC_USBS = x1_BIT, UPM: USART_UCSRC_UPM = DISABLED, UMSEL0: bool = false) =
  var x: uint8
...
func UBRR*(r: USART_UBRR_Fields): uint16 {.inline.} =
  r.uint16.bitsliced(0 .. 11)

proc `UBRR=`*(r: var USART_UBRR_Fields, val: uint16) {.inline.} =
  var tmp = r.uint16
  tmp.clearMask(0 .. 11)
  tmp.setMask((val shl 0).masked(0 .. 11))
  r = tmp.USART_UBRR_Fields
...
func UMSEL0*(r: USART_UCSRC_Fields): bool {.inline.} =
  r.uint8.bitsliced(6 .. 6).bool

proc `UMSEL0=`*(r: var USART_UCSRC_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 6)
  tmp.setMask((val.uint8 shl 6).masked(6 .. 6))
  r = tmp.USART_UCSRC_Fields
