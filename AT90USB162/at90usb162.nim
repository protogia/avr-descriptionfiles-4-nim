# Peripheral access API for AT90USB162 microcontrollers (generated using svd2nim)
...
type PS2_PS2CON_Type* = object
  loc: uint

type PS2_UPOE_Type* = object
  loc: uint

type PS2_Type* = object
  PS2CON*: PS2_PS2CON_Type
  UPOE*: PS2_UPOE_Type
...
const PS2* = PS2_Type(
  PS2CON: PS2_PS2CON_Type(loc: 0xfa'u),
  UPOE: PS2_UPOE_Type(loc: 0xfb'u),
)
...
proc write*(reg: PORTC_DDRC_Type, DDC: uint8 = 0) =
  var x: uint8
  x.setMask((DDC shl 0).masked(0 .. 2))
...
proc write*(reg: PORTC_PINC_Type, PINC: uint8 = 0) =
  var x: uint8
  x.setMask((PINC shl 0).masked(0 .. 2))
...
proc write*(reg: PORTC_PORTC_Type, PORTC: uint8 = 0) =
  var x: uint8
  x.setMask((PORTC shl 0).masked(0 .. 2))
...
func DDC*(r: PORTC_DDRC_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 2)

proc `DDC=`*(r: var PORTC_DDRC_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 2)
  tmp.setMask((val shl 0).masked(0 .. 2))
  r = tmp.PORTC_DDRC_Fields

func PINC*(r: PORTC_PINC_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 2)

proc `PINC=`*(r: var PORTC_PINC_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 2)
  tmp.setMask((val shl 0).masked(0 .. 2))
  r = tmp.PORTC_PINC_Fields

func PORTCField*(r: PORTC_PORTC_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(0 .. 2)

proc `PORTCField=`*(r: var PORTC_PORTC_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 2)
  tmp.setMask((val shl 0).masked(0 .. 2))
...
type
  PS2_PS2CON_Fields* = distinct uint8
  PS2_UPOE_Fields* = distinct uint8

proc read*(reg: PS2_PS2CON_Type): PS2_PS2CON_Fields {.inline.} =
  volatileLoad(cast[ptr PS2_PS2CON_Fields](reg.loc))

proc write*(reg: PS2_PS2CON_Type, val: PS2_PS2CON_Fields) {.inline.} =
  volatileStore(cast[ptr PS2_PS2CON_Fields](reg.loc), val)

proc write*(reg: PS2_PS2CON_Type, PS2EN: bool = false) =
  var x: uint8
  x.setMask((PS2EN.uint8 shl 0).masked(0 .. 0))
  reg.write x.PS2_PS2CON_Fields

template modifyIt*(reg: PS2_PS2CON_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

proc read*(reg: PS2_UPOE_Type): PS2_UPOE_Fields {.inline.} =
  volatileLoad(cast[ptr PS2_UPOE_Fields](reg.loc))

proc write*(reg: PS2_UPOE_Type, val: PS2_UPOE_Fields) {.inline.} =
  volatileStore(cast[ptr PS2_UPOE_Fields](reg.loc), val)

proc write*(reg: PS2_UPOE_Type, DMI: bool = false, DPI: bool = false, DATAI: bool = false, SCKI: bool = false, UPDRV: uint8 = 0, UPWE: uint8 = 0) =
  var x: uint8
  x.setMask((DMI.uint8 shl 0).masked(0 .. 0))
  x.setMask((DPI.uint8 shl 1).masked(1 .. 1))
  x.setMask((DATAI.uint8 shl 2).masked(2 .. 2))
  x.setMask((SCKI.uint8 shl 3).masked(3 .. 3))
  x.setMask((UPDRV shl 4).masked(4 .. 5))
  x.setMask((UPWE shl 6).masked(6 .. 7))
  reg.write x.PS2_UPOE_Fields

template modifyIt*(reg: PS2_UPOE_Type, op: untyped): untyped =
  block:
    var it {.inject.} = reg.read()
    op
    reg.write(it)

func PS2EN*(r: PS2_PS2CON_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `PS2EN=`*(r: var PS2_PS2CON_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.PS2_PS2CON_Fields

func DMI*(r: PS2_UPOE_Fields): bool {.inline.} =
  r.uint8.bitsliced(0 .. 0).bool

proc `DMI=`*(r: var PS2_UPOE_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(0 .. 0)
  tmp.setMask((val.uint8 shl 0).masked(0 .. 0))
  r = tmp.PS2_UPOE_Fields

func DPI*(r: PS2_UPOE_Fields): bool {.inline.} =
  r.uint8.bitsliced(1 .. 1).bool

proc `DPI=`*(r: var PS2_UPOE_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(1 .. 1)
  tmp.setMask((val.uint8 shl 1).masked(1 .. 1))
  r = tmp.PS2_UPOE_Fields

func DATAI*(r: PS2_UPOE_Fields): bool {.inline.} =
  r.uint8.bitsliced(2 .. 2).bool

proc `DATAI=`*(r: var PS2_UPOE_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(2 .. 2)
  tmp.setMask((val.uint8 shl 2).masked(2 .. 2))
  r = tmp.PS2_UPOE_Fields

func SCKI*(r: PS2_UPOE_Fields): bool {.inline.} =
  r.uint8.bitsliced(3 .. 3).bool

proc `SCKI=`*(r: var PS2_UPOE_Fields, val: bool) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(3 .. 3)
  tmp.setMask((val.uint8 shl 3).masked(3 .. 3))
  r = tmp.PS2_UPOE_Fields

func UPDRV*(r: PS2_UPOE_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(4 .. 5)

proc `UPDRV=`*(r: var PS2_UPOE_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(4 .. 5)
  tmp.setMask((val shl 4).masked(4 .. 5))
  r = tmp.PS2_UPOE_Fields

func UPWE*(r: PS2_UPOE_Fields): uint8 {.inline.} =
  r.uint8.bitsliced(6 .. 7)

proc `UPWE=`*(r: var PS2_UPOE_Fields, val: uint8) {.inline.} =
  var tmp = r.uint8
  tmp.clearMask(6 .. 7)
  tmp.setMask((val shl 6).masked(6 .. 7))
  r = tmp.PS2_UPOE_Fields
